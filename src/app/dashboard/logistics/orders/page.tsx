    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(20,18,12,0.45)", display: "flex", justifyContent: "flex-end", zIndex: 50, backdropFilter: "blur(2px)" }}>
      <div onClick={e => e.stopPropagation()} style={{ width: "min(540px, 96vw)", background: "var(--bg-panel)", boxShadow: "-12px 0 32px rgba(0,0,0,.14)", display: "flex", flexDirection: "column", height: "100%", borderLeft: "1px solid var(--line-1)" }}>
        <div style={{ padding: "18px 22px 14px", borderBottom: "1px solid var(--line-1)", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 600, letterSpacing: "-0.01em" }}>{editing ? "Editar orden" : "Nueva orden"}</div>
            <div style={{ fontSize: 12, color: "var(--ink-3)", marginTop: 1 }}>Orden de trabajo y entrega de material</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 22px 0" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {!form.eventId && (
              <div>
                <FL>Nombre del evento *</FL>
                <input style={INP} value={form.eventName} onChange={e => set("eventName", e.target.value)} placeholder="Ej. Boda Cami & Juanca" />
              </div>
            )}

            <div>
              <FL>Cliente</FL>
              <input style={INP} value={form.clientName} onChange={e => set("clientName", e.target.value)} placeholder="Nombre del cliente" />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div>
                <FL>Email</FL>
                <input style={INP} type="email" value={form.clientEmail} onChange={e => set("clientEmail", e.target.value)} placeholder="email@ejemplo.com" />
              </div>
              <div>
                <FL>Teléfono</FL>
                <input style={INP} type="tel" value={form.clientPhone} onChange={e => set("clientPhone", e.target.value)} placeholder="+34 600 000 000" />
              </div>
            </div>

            <DrawerSection label="2 — Estado y referencia" />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div>
                <FL>Estado</FL>
              ...
              <div>
                <FL>Documento de referencia</FL>
                <input style={INP} value={form.linkedDocId} onChange={e => set("linkedDocId", e.target.value)} placeholder="PRE-2026-001, FAC-001…" />
              </div>
            </div>

            <DrawerSection label="3 — Materiales a entregar" />
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <div key={idx} style={{ border: "1px solid var(--line-1)", borderRadius: "var(--r-sm)", padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
                  <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
                    <div style={{ flex: 1 }}>
                      ...
                      {!item.productId && (
                        <input style={{ ...INP, fontSize: 12.5, marginTop: 4 }} value={item.productName} onChange={e => setItem(idx, "productName", e.target.value)} placeholder="O escribe el nombre…" />
                      )}
                    </div>
                    <div style={{ width: 68 }}>
                      <FL>Cant.</FL>
                  ...
                  <input style={{ ...INP, fontSize: 12.5 }} value={item.description || ""} onChange={e => setItem(idx, "description", e.target.value)} placeholder="Descripción o instrucciones…" />
                  <label style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", fontSize: 12, color: "var(--ink-2)" }}>
                    ...
                    Añadir orden de servicio para este producto
                  </label>
                  {item.hasService && (
                    <div style={{ borderTop: "1px solid var(--line-1)", paddingTop: 8, display: "flex", flexDirection: "column", gap: 8 }}>
                      <div>
                        <FL>Título del servicio *</FL>
                        <input style={{ ...INP, fontSize: 12.5 }} value={item.serviceTitle || ""} onChange={e => setItem(idx, "serviceTitle", e.target.value)} placeholder="Ej. Montaje y desmontaje de tarima" />
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                        <div><FL>Hora inicio</FL><input style={{ ...INP, fontSize: 12.5 }} type="time" value={item.serviceTimeFrom || ""} onChange={e => setItem(idx, "serviceTimeFrom", e.target.value)} /></div>
                        <div><FL>Hora fin</FL><input style={{ ...INP, fontSize: 12.5 }} type="time" value={item.serviceTimeTo || ""} onChange={e => setItem(idx, "serviceTimeTo", e.target.value)} /></div>
                        <div><FL>Trabajador</FL><input style={{ ...INP, fontSize: 12.5 }} value={item.serviceWorkerName || ""} onChange={e => setItem(idx, "serviceWorkerName", e.target.value)} placeholder="Nombre" /></div>
                      </div>
                      <div><FL>Ubicación</FL><input style={{ ...INP, fontSize: 12.5 }} value={item.serviceLocation || ""} onChange={e => setItem(idx, "serviceLocation", e.target.value)} placeholder="Sala, zona o acceso" /></div>
                      <div>
                        <FL>Notas de servicio</FL>
                        <textarea style={{ ...INP, fontSize: 12.5, resize: "vertical", minHeight: 56 } as React.CSSProperties} value={item.serviceNotes || ""} onChange={e => setItem(idx, "serviceNotes", e.target.value)} placeholder="Instrucciones especiales…" />
                      </div>
                    </div>
                  )}
                </div>
              ...
              <button onClick={addItem} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "8px 12px", border: "1px dashed var(--line-strong)", borderRadius: "var(--r-sm)", background: "transparent", color: "var(--ink-3)", fontSize: 12.5, cursor: "pointer" }}>
                <RiAddLine size={14} /> Añadir producto
              </button>
            </div>

            <DrawerSection label="Notas internas" />
            <div style={{ paddingBottom: 20 }}>
              <textarea style={{ ...INP, resize: "vertical", minHeight: 70 } as React.CSSProperties} value={form.notes} onChange={e => set("notes", e.target.value)} placeholder="Notas internas sobre esta orden…" />
            </div>
          </div>
        </div>
        ...
        <div style={{ position: "absolute", top: "calc(100% + 4px)", right: 0, background: "var(--bg-panel)", border: "1px solid var(--line-1)", borderRadius: "var(--r-md)", boxShadow: "0 6px 18px rgba(0,0,0,.13)", zIndex: 40, minWidth: 160, overflow: "hidden", padding: 4 }}>
          ...
          {confirmDel ? (
            <div style={{ display: "flex", gap: 4, padding: "4px 6px" }}>
              <button onClick={() => { onDelete(); setOpen(false); }} style={{ flex: 1, padding: "6px 8px", background: "#FBEDEC", border: "none", borderRadius: "var(--r-sm)", color: "#B55450", fontSize: 12, cursor: "pointer", fontWeight: 500 }}>Confirmar</button>
              <button onClick={() => setConfirmDel(false)} style={{ flex: 1, padding: "6px 8px", background: "var(--bg-subtle)", border: "none", borderRadius: "var(--r-sm)", color: "var(--ink-2)", fontSize: 12, cursor: "pointer" }}>Cancelar</button>
            </div>
          ) : (
            <button onClick={() => setConfirmDel(true)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", background: "none", border: "none", cursor: "pointer", fontSize: 12.5, color: "#B55450", borderRadius: "var(--r-sm)" }}>
              <RiDeleteBinLine size={13} /> Eliminar
            </button>
          )}
        </div>
      )}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14, height: "100%" }}>
      ...
      {orders.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
          {[
            { label: "Total órdenes",     value: orders.length, sub: "registradas",              color: "var(--ink-1)" },
            { label: "Sólo carga",        value: soloCart.length, sub: `${orders.length ? Math.round(soloCart.length/orders.length*100) : 0}% del total`, color: "#4A6A94" },
            { label: "Carga + Servicio",  value: cartSvc.length,  sub: `${orders.length ? Math.round(cartSvc.length/orders.length*100) : 0}% del total`,  color: "#7B5EA7" },
            { label: "Entregadas",        value: delivered.length, sub: `${orders.length ? Math.round(delivered.length/orders.length*100) : 0}% del total`, color: "#4F7A5E" },
          ].map(s => (
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por evento, cliente o referencia…" style={{ ...INP, paddingLeft: 30 }} />
        </div>
        <button
        ...
        >
          <RiAddLine size={14} /> Crear Orden
        </button>
      </div>

      {/* ── Table ── */}
      {loading ? (
        <div style={{ textAlign: "center", padding: 48, color: "var(--ink-3)", fontSize: 13 }}>Cargando órdenes…</div>
      ) : orders.length === 0 ? (
        <div style={{ background: "var(--bg-panel)", border: "1px solid var(--line-1)", borderRadius: "var(--r-md)", padding: 56, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 600, color: "var(--ink-1)" }}>Sin órdenes</div>
          <div style={{ fontSize: 13, color: "var(--ink-3)", maxWidth: 360, lineHeight: 1.5 }}>Crea tu primera orden de trabajo para gestionar la entrega de material a eventos.</div>
        ...
        <div style={{ background: "var(--bg-panel)", border: "1px solid var(--line-1)", borderRadius: "var(--r-md)", overflow: "hidden" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: COL, background: "var(--bg-subtle)", borderBottom: "1px solid var(--line-1)" }}>
            ...
            <div style={{ ...TH("CLIENTE"), borderLeft: "none" }}>Cliente</div>
            <div style={TH("FECHA", true)}>Fecha</div>
            <div style={TH("NÚMERO", true)}>Número</div>
            <div style={TH("CONTENIDO")}>Contenido</div>
            <div style={TH("ESTADO", true)}>Estado</div>
          ...
          {filtered.length > 0 && (
            <div style={{ padding: "8px 12px", borderTop: "1px solid var(--line-1)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "var(--bg-subtle)" }}>
              <span style={{ fontSize: 12, color: "var(--ink-3)" }}>
                {filtered.length} {filtered.length === 1 ? "orden" : "órdenes"}{selected.size > 0 && ` · ${selected.size} seleccionada${selected.size > 1 ? "s" : ""}`}
              </span>
