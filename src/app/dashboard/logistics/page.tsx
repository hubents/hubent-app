      ...
      <div onClick={e => e.stopPropagation()} style={{ width: 480, background: "var(--bg-panel)", borderTopLeftRadius: 16, borderBottomLeftRadius: 16, padding: "22px 24px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", marginBottom: 4 }}>
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 17, fontWeight: 600 }}>{editing ? "Editar producto" : "Nuevo producto"}</div>
              <div style={{ fontSize: 12, color: "var(--ink-3)" }}>Catálogo de productos</div>
            </div>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div className="form-field">
            <label>Tipo *</label>
            <select value={form.type} onChange={e => { const t = e.target.value as ProductType; set("type", t); if (t === "servicio") set("subtype", "servicio"); }}>
          ...
          <div className="form-field">
            ...
            <select value={form.subtype} onChange={e => set("subtype", e.target.value)} disabled={form.type === "servicio"}>
              {form.type !== "servicio" && <option value="alquiler">Alquiler</option>}
              {form.type !== "servicio" && <option value="venta">Venta</option>}
              <option value="servicio">Servicio</option>
            </select>
          </div>
        </div>
        ...
        <div className="form-field"><label>Nombre *</label><input value={form.name} onChange={e => set("name", e.target.value)} placeholder="Ej. Silla Tiffany Blanca" /></div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div className="form-field"><label>SKU</label><input value={form.sku} onChange={e => set("sku", e.target.value)} placeholder="MOB-SIL-01" /></div>
          <div className="form-field">
        ...
        <div className="form-field"><label>Descripción</label><textarea value={form.description} onChange={e => set("description", e.target.value)} placeholder="Descripción..." style={{ resize: "vertical", minHeight: 60 }} /></div>

        <Divider label="Precios" />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div className="form-field"><label>Coste (€)</label><input type="number" min="0" step="0.01" value={form.cost} onChange={e => set("cost", e.target.value)} placeholder="0.00" /></div>
          <div className="form-field"><label>Venta s/IVA (€)</label><input type="number" min="0" step="0.01" value={form.unitPrice} onChange={e => set("unitPrice", e.target.value)} placeholder="0.00" /></div>
        </div>
        <div className="form-field">
          <label>IVA (%)</label>
        ...
        {form.type !== "servicio" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div className="form-field"><label>Stock actual</label><input type="number" min="0" value={form.stock} onChange={e => set("stock", e.target.value)} placeholder="0" /></div>
              <div className="form-field"><label>Stock mínimo</label><input type="number" min="0" value={form.stockMin} onChange={e => set("stockMin", e.target.value)} placeholder="0" /></div>
            </div>
            <div style={{ background: "#EEF3FF", border: "1px solid #C5D4F0", borderRadius: 8, padding: "10px 12px", fontSize: 12, color: "#3A5B8A", display: "flex", gap: 8, alignItems: "flex-start" }}>
              ...
              <span>Al añadir este producto a presupuestos y facturas, el stock se gestionará por disponibilidad de fecha, evitando overbooking de alquiler.</span>
            </div>
          </>
        )}

        <Divider label="Color del avatar" />
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <span style={{ fontSize: 12, color: "var(--ink-3)" }}>Vista previa</span>
        </div>

        <div style={{ display: "flex", gap: 8, marginTop: 14, justifyContent: "flex-end", paddingTop: 12, borderTop: "1px solid var(--line-1)" }}>
          <button onClick={onClose} style={{ padding: "8px 16px", borderRadius: "var(--r-sm)", border: "1px solid var(--line-1)", background: "var(--bg-panel)", color: "var(--ink-2)", fontSize: 13, cursor: "pointer" }}>Cancelar</button>
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: 440, background: "var(--bg-panel)", borderTopLeftRadius: 16, borderBottomLeftRadius: 16, padding: "22px 24px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", marginBottom: 4 }}>
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ fontSize: 17, fontWeight: 600 }}>{editing ? "Editar almacén" : "Nuevo almacén"}</div>
          </div>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--ink-3)", padding: 4, borderRadius: 6, display: "flex" }}><IcoX /></button>
        </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {(["fijo", "movil"] as const).map(t => (
            <button key={t} onClick={() => set("type", t)} style={{ flex: 1, padding: "9px 0", borderRadius: "var(--r-sm)", border: `1.5px solid ${form.type === t ? "var(--ink-1)" : "var(--line-1)"}`, background: form.type === t ? "var(--bg-subtle)" : "var(--bg-panel)", color: form.type === t ? "var(--ink-1)" : "var(--ink-3)", fontSize: 13, fontWeight: form.type === t ? 600 : 400, cursor: "pointer" }}>
              {t === "fijo" ? "🏭 Almacén fijo" : "🚐 Furgoneta"}
            </button>
          ))}
        </div>

        <Divider label="Información" />
        <div className="form-field"><label>Nombre *</label><input value={form.name} onChange={e => set("name", e.target.value)} placeholder={form.type === "fijo" ? "Almacén Principal" : "Furgoneta 1"} /></div>
        {form.type === "fijo" ? (
          <>
            <div className="form-field"><label>Dirección</label><input value={form.location} onChange={e => set("location", e.target.value)} placeholder="C/ Ejemplo 1, Ciudad" /></div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div className="form-field"><label>Capacidad (m²)</label><input type="number" min="0" value={form.capacity} onChange={e => set("capacity", e.target.value)} placeholder="0" /></div>
              <div className="form-field"><label>Responsable</label><input value={form.manager} onChange={e => set("manager", e.target.value)} placeholder="Nombre" /></div>
            </div>
          </>
        ) : (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div className="form-field"><label>Matrícula</label><input value={form.plate} onChange={e => set("plate", e.target.value)} placeholder="MA-1234-AB" /></div>
              <div className="form-field"><label>Conductor</label><input value={form.driver} onChange={e => set("driver", e.target.value)} placeholder="Nombre conductor" /></div>
            </div>
            <div className="form-field"><label>Capacidad (unidades)</label><input type="number" min="0" value={form.capacity} onChange={e => set("capacity", e.target.value)} placeholder="0" /></div>
          </>
        )}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(20,18,12,0.35)", zIndex: 60, display: "flex", justifyContent: "flex-end" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width: 520, background: "var(--bg-panel)", borderTopLeftRadius: 16, borderBottomLeftRadius: 16, display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        <div style={{ padding: "18px 22px", borderBottom: "1px solid var(--line-1)", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 600 }}>Cargar furgoneta</div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 22px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* Event picker */}
          <div>
            <Divider label="Selecciona el evento" />
            ...
            {loadingEvents ? (
              <div style={{ textAlign: "center", padding: "24px 0", color: "var(--ink-3)", fontSize: 12.5 }}>Cargando eventos...</div>
            ) : filteredEvents.length === 0 ? (
              <div style={{ textAlign: "center", padding: "24px 0", color: "var(--ink-3)", fontSize: 12.5 }}>Sin eventos{eventQ ? " que coincidan" : ""}</div>
            ) : (
          ...
          {selectedEvent && (
            <>
              <Divider label="Materiales a cargar" />
              {items.map((item, idx) => (
              ...
              <button onClick={addItem} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 12px", border: "1px dashed var(--line-1)", borderRadius: "var(--r-sm)", background: "transparent", color: "var(--ink-3)", fontSize: 12, cursor: "pointer", alignSelf: "flex-start" }}>
                <IcoPlus size={12} /> Añadir producto
              </button>

              <Divider label="Notas" />
              <div className="form-field">
                <label>Notas internas</label>
                <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Instrucciones de carga, observaciones..." style={{ resize: "vertical", minHeight: 60 }} />
              </div>
            </>
          )}
        </div>
        ...
        <div style={{ padding: "18px 22px", borderBottom: "1px solid var(--line-1)", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12, color: "var(--ink-3)", marginTop: 1 }}>
              {warehouse.type === "fijo"
                ? warehouse.location || "Ubicación no especificada"
                : `${warehouse.plate || "Sin matrícula"}${warehouse.driver ? ` · ${warehouse.driver}` : ""}`}
            </div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 22px", display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
            <div style={{ background: "var(--bg-subtle)", borderRadius: "var(--r-sm)", padding: "12px 14px", border: "1px solid var(--line-1)" }}>
              <div style={{ fontSize: 10.5, color: "var(--ink-3)", fontWeight: 500, marginBottom: 4 }}>Tipo</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "var(--ink-1)" }}>{warehouse.type === "movil" ? "🚐 Móvil" : "🏭 Fijo"}</div>
            </div>
            <div style={{ background: "var(--bg-subtle)", borderRadius: "var(--r-sm)", padding: "12px 14px", border: "1px solid var(--line-1)" }}>
              <div style={{ fontSize: 10.5, color: "var(--ink-3)", fontWeight: 500, marginBottom: 4 }}>Productos</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "var(--ink-1)" }}>{whProducts.length}</div>
            </div>
            <div style={{ background: "var(--bg-subtle)", borderRadius: "var(--r-sm)", padding: "12px 14px", border: "1px solid var(--line-1)" }}>
              <div style={{ fontSize: 10.5, color: "var(--ink-3)", fontWeight: 500, marginBottom: 4 }}>Capacidad</div>
          ...
          {warehouse.type === "movil" && (
            <div>
              <div style={{ fontSize: 11.5, fontWeight: 600, color: "var(--ink-3)", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.04em" }}>Evento actual</div>
              <div style={{ padding: "12px 14px", borderRadius: "var(--r-sm)", background: warehouse.eventName ? "#F0EBF8" : "var(--bg-subtle)", border: `1px solid ${warehouse.eventName ? "#C9B8E8" : "var(--line-1)"}` }}>
                {warehouse.eventName ? (
                  ...
                  <div style={{ fontSize: 12.5, color: "var(--ink-3)" }}>Sin evento asignado</div>
                )}
              </div>
            </div>
          )}
          ...
          <div>
            <div style={{ fontSize: 11.5, fontWeight: 600, color: "var(--ink-3)", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.04em" }}>
              {warehouse.type === "movil" ? "Materiales cargados" : "Inventario"}
            </div>
            {whProducts.length === 0 ? (
              <div style={{ textAlign: "center", padding: "24px 0", color: "var(--ink-3)", fontSize: 12.5 }}>Sin productos asignados</div>
            ) : (
                  ...
                  return (
                    <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderBottom: idx < whProducts.length - 1 ? "1px solid var(--line-1)" : "none" }}>
                      ...
                      <div style={{ textAlign: "right", flexShrink: 0 }}>
                        ...
                        {p.stockMin != null && <div style={{ fontSize: 10.5, color: "var(--ink-3)" }}>mín {p.stockMin}</div>}
                      </div>
                      {low && (
          ...
          {/* Recent movements */}
          <div>
            <div style={{ fontSize: 11.5, fontWeight: 600, color: "var(--ink-3)", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.04em" }}>Movimientos recientes</div>
            {loadingMov ? (
              <div style={{ fontSize: 12.5, color: "var(--ink-3)", padding: "12px 0" }}>Cargando...</div>
            ) : movements.length === 0 ? (
              <div style={{ textAlign: "center", padding: "20px 0", color: "var(--ink-3)", fontSize: 12.5 }}>Sin movimientos registrados</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 0, border: "1px solid var(--line-1)", borderRadius: "var(--r-sm)", overflow: "hidden" }}>
                {movements.map((m, idx) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(30,25,20,0.28)", display: "flex", justifyContent: "flex-end", zIndex: 100 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width: 480, background: "var(--bg-panel)", boxShadow: "-12px 0 32px rgba(0,0,0,.14)", display: "flex", flexDirection: "column", height: "100%", borderLeft: "1px solid var(--line-1)" }}>
        <div style={{ padding: "18px 22px", borderBottom: "1px solid var(--line-1)", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 600 }}>Nuevo movimiento</div>
          <button onClick={onClose} style={{ marginLeft: "auto", background: "none", border: "none", cursor: "pointer", color: "var(--ink-3)", padding: 6, display: "flex" }}><IcoX size={18} /></button>
        </div>

        <div style={{ flex: 1, overflowY: "auto", padding: "20px 22px", display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <div style={{ fontSize: 11.5, fontWeight: 500, color: "var(--ink-3)", marginBottom: 8 }}>Tipo de movimiento</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div className="form-field"><label>Cantidad *</label><input type="number" min="1" value={qty} onChange={e => setQty(e.target.value)} /></div>
            <div className="form-field"><label>Fecha</label><input type="date" value={date} onChange={e => setDate(e.target.value)} /></div>
          </div>

          {type === "transferencia" ? (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div className="form-field"><label>Origen</label><select value={fromId} onChange={e => setFromId(e.target.value)}>{whs.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}</select></div>
              <div className="form-field"><label>Destino</label><select value={toId} onChange={e => setToId(e.target.value)}>{whs.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}</select></div>
            </div>
          ) : (
            <div className="form-field">
              <label>Almacén</label>
          ...
          {type === "reserva" && (
            <>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div className="form-field"><label>Hora inicio</label><input type="time" value={timeFrom} onChange={e => setTimeFrom(e.target.value)} /></div>
                <div className="form-field"><label>Hora fin</label><input type="time" value={timeTo} onChange={e => setTimeTo(e.target.value)} /></div>
              </div>
              <div className="form-field"><label>Evento</label><input value={eventName} onChange={e => setEventName(e.target.value)} placeholder="Ej. Boda García" /></div>
            </>
          )}

          <div className="form-field"><label>Referencia</label><input value={reference} onChange={e => setReference(e.target.value)} placeholder="OC-2026-001" /></div>
          <div className="form-field"><label>Notas</label><textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Observaciones..." style={{ resize: "vertical", minHeight: 60 }} /></div>
        </div>
...
function ReservationDrawer({ initial, warehouses: whs, products, onSave, onClose }: {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(20,18,12,0.35)", zIndex: 60, display: "flex", justifyContent: "flex-end" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width: 520, background: "var(--bg-panel)", borderTopLeftRadius: 16, borderBottomLeftRadius: 16, display: "flex", flexDirection: "column", height: "100%" }}>
        <div style={{ padding: "18px 22px", borderBottom: "1px solid var(--line-1)", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 600 }}>{editing ? "Editar reserva" : "Nueva reserva"}</div>
          <button onClick={onClose} style={{ marginLeft: "auto", background: "none", border: "none", cursor: "pointer", color: "var(--ink-3)", padding: 6, display: "flex" }}><IcoX size={18} /></button>
        </div>

        <div style={{ flex: 1, overflowY: "auto", padding: "20px 22px", display: "flex", flexDirection: "column", gap: 14 }}>
          <Divider label="Evento" />
          <div className="form-field"><label>Nombre del evento *</label><input value={form.eventName} onChange={e => set("eventName", e.target.value)} placeholder="Ej. Boda García-López" /></div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div className="form-field"><label>Fecha</label><input type="date" value={form.date} onChange={e => set("date", e.target.value)} /></div>
            <div className="form-field">
              <label>Estado</label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div className="form-field"><label>Hora inicio</label><input type="time" value={form.timeFrom} onChange={e => set("timeFrom", e.target.value)} /></div>
            <div className="form-field"><label>Hora fin</label><input type="time" value={form.timeTo} onChange={e => set("timeTo", e.target.value)} /></div>
          </div>

          <Divider label="Ubicación" />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div className="form-field"><label>Sede / Venue</label><input value={form.venue} onChange={e => set("venue", e.target.value)} placeholder="Nombre del espacio" /></div>
            <div className="form-field"><label>Ciudad</label><input value={form.venueCity} onChange={e => set("venueCity", e.target.value)} placeholder="Ciudad" /></div>
          </div>

          <Divider label="Transporte" />
          <div className="form-field">
            <label>Vehículo / Almacén</label>
            <select value={form.warehouseId} onChange={e => set("warehouseId", e.target.value)}>
              <option value="">Sin asignar</option>
              {whs.map(w => <option key={w.id} value={w.id}>{w.name} ({w.type === "movil" ? "Furgoneta" : "Fijo"})</option>)}
            </select>
          </div>
          <div className="form-field">
            <label>Ubicación de los materiales</label>
            <input value={form.itemsLocation} onChange={e => set("itemsLocation", e.target.value)} placeholder="En furgoneta / En almacén" />
          </div>

          <Divider label="Materiales" />
          {items.map((item, idx) => (
            <div key={idx} style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <div style={{ flex: 1 }} className="form-field">
                {idx === 0 && <label>Producto</label>}
          ...
          <Divider label="Notas" />
          <div className="form-field"><label>Notas internas</label><textarea value={form.notes} onChange={e => set("notes", e.target.value)} placeholder="Observaciones..." style={{ resize: "vertical", minHeight: 60 }} /></div>
        </div>
...
function WarehousesTab({ warehouses: whs, products, onRefetch, onNewMovement, openNew, onNewConsumed }: {
  ...
  const unassignEvent = async (w: Warehouse) => {
    ...
    toast.success("Evento desasignado");
    onRefetch();
  };
  ...
  if (whs.length === 0) {
    return (
      <div style={{ textAlign: "center", padding: "60px 0", color: "var(--ink-3)" }}>
        <IcoWh size={32} />
        <div style={{ marginTop: 12, fontSize: 14, fontWeight: 500 }}>Sin almacenes</div>
        <div style={{ fontSize: 12, marginTop: 4 }}>Usa el botón «Nuevo almacén» de arriba para crear el primero</div>
      </div>
    );
  }
        ...
        {whs.map(w => {
          ...
          return (
            <div key={w.id} style={{ background: "var(--bg-panel)", border: "1px solid var(--line-1)", borderRadius: "var(--r-md)", padding: "18px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {isMobil && (
                <div style={{ padding: "8px 12px", borderRadius: "var(--r-sm)", background: w.eventName ? "#F0EBF8" : "var(--bg-subtle)", border: `1px solid ${w.eventName ? "#C9B8E8" : "var(--line-1)"}`, display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <div style={{ flex: 1, fontSize: 12.5, color: w.eventName ? "#7B5EA7" : "var(--ink-3)", fontWeight: w.eventName ? 500 : 400 }}>
                    {w.eventName || "Sin evento asignado"}
                  </div>
                  {w.eventName && (
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--ink-3)", marginBottom: 6 }}>
                  <span>Ocupación ({whProducts.length} {whProducts.length === 1 ? "producto" : "productos"})</span>
                  <span style={{ fontWeight: 500 }}>{totalStock} / {capacity || "∞"} uds.</span>
                </div>
              ...
              <div style={{ display: "flex", gap: 6, paddingTop: 4, borderTop: "1px solid var(--line-1)" }}>
                ...
                {isMobil && (
                  <button onClick={() => setLoadWarehouse(w)} style={{ flex: 1, padding: "7px", background: wColor + "15", border: `1px solid ${wColor}44`, borderRadius: "var(--r-sm)", fontSize: 12, cursor: "pointer", color: wColor, fontWeight: 500 }}>
                    {w.eventName ? "Editar carga" : "Cargar furgoneta"}
                  </button>
                )}
                <button onClick={onNewMovement} title="Nuevo movimiento" style={{ padding: "7px 10px", background: "var(--bg-subtle)", border: "1px solid var(--line-1)", borderRadius: "var(--r-sm)", fontSize: 12, cursor: "pointer", color: "var(--ink-2)" }}>
                  + Mov.
                </button>
      ...
      {loadWarehouse && (
        <LoadEventDrawer
          ...
          onSave={async (data) => {
            ...
            toast.success("Furgoneta cargada y reserva creada");
          ...
          <table className="tbl">
            <thead>
              <tr>
                ...
                <th style={{ textAlign: "right", color: "#7B5EA7" }}>Reservado</th>
                <th style={{ textAlign: "right", color: "#C4874A" }}>En ruta</th>
                <th>Ubicación</th>
                <th>Estado</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={8} style={{ textAlign: "center", padding: "40px 20px", color: "var(--ink-3)", fontSize: 13 }}>No hay productos que coincidan</td></tr>
              )}
              {filtered.map(p => {
                const low = p.stock != null && p.stockMin != null && p.stock <= p.stockMin;
                return (
...
function MovementsTab({ products, warehouses: whs, movementDrawerOpen, onMovementDrawerClose }: {
  ...
  if (loading) return <div style={{ textAlign: "center", padding: "60px 0", color: "var(--ink-3)" }}>Cargando movimientos...</div>;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 0", color: "var(--ink-3)" }}>
          <IcoMov size={32} />
          <div style={{ marginTop: 12, fontSize: 14, fontWeight: 500 }}>Sin movimientos registrados</div>
        </div>
      ) : (
        <div style={{ background: "var(--bg-panel)", border: "1px solid var(--line-1)", borderRadius: 14, overflow: "hidden" }}>
          {filtered.map((m, idx) => {
...
function ReservasTab({ warehouses: whs, products }: { warehouses: Warehouse[]; products: Product[] }) {
  ...
  if (loading) return <div style={{ textAlign: "center", padding: "60px 0", color: "var(--ink-3)" }}>Cargando reservas...</div>;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 0", color: "var(--ink-3)" }}>
          <IcoMov size={32} />
          <div style={{ marginTop: 12, fontSize: 14, fontWeight: 500 }}>Sin reservas</div>
          <div style={{ fontSize: 12, marginTop: 4 }}>Crea tu primera reserva de material para un evento</div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {filtered.map(r => {
            const st = STATUS_STYLE[r.status || "pendiente"] || STATUS_STYLE.pendiente;
            const locationLabel = r.warehouseType === "movil" ? "En furgoneta" : r.warehouseType === "fijo" ? "En almacén" : r.itemsLocation || "—";
            return (
              <div key={r.id} style={{ background: "var(--bg-panel)", border: "1px solid var(--line-1)", borderRadius: 14, padding: 16 }}>
                {/* Header */}
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: "var(--ink-1)" }}>{r.eventName || "Sin nombre"}</span>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  ...
                  {r.status === "en_ruta" && (
                    <button onClick={() => updateStatus(r.id, "entregada")} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: "var(--r-sm)", border: "1px solid #4F7A5E", background: "transparent", color: "#4F7A5E", fontSize: 12, cursor: "pointer" }}>
                      <IcoDone size={13} /> Entregar en sede
                    </button>
                  )}
                  {r.status === "entregada" && (
    ...
    <div style={{ padding: "24px 28px", maxWidth: 1280, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 16 }}>
        <div style={{ background: "var(--bg-panel)", border: "1px solid var(--line-1)", borderRadius: "var(--r-md)", padding: "16px 18px" }}>
          <div style={{ fontSize: 11.5, color: "var(--ink-3)", fontWeight: 500, marginBottom: 6 }}>Almacenes fijos</div>
          <div style={{ fontSize: 26, fontWeight: 700, color: "#4F7A5E", letterSpacing: "-0.02em", lineHeight: 1 }}>{fixedWarehouses.length}</div>
          <div style={{ fontSize: 11.5, color: "var(--ink-3)", marginTop: 4 }}>ubicaciones físicas</div>
        </div>
        <div style={{ background: "var(--bg-panel)", border: "1px solid var(--line-1)", borderRadius: "var(--r-md)", padding: "16px 18px" }}>
          <div style={{ fontSize: 11.5, color: "var(--ink-3)", fontWeight: 500, marginBottom: 6 }}>Furgonetas</div>
        ...
        <div style={{ background: "var(--bg-panel)", border: "1px solid var(--line-1)", borderRadius: "var(--r-md)", padding: "16px 18px" }}>
          <div style={{ fontSize: 11.5, color: "var(--ink-3)", fontWeight: 500, marginBottom: 6 }}>Reservas activas</div>
          <div style={{ fontSize: 26, fontWeight: 700, color: "#7B5EA7", letterSpacing: "-0.02em", lineHeight: 1 }}>—</div>
          <div style={{ fontSize: 11.5, color: "var(--ink-3)", marginTop: 4 }}>confirmadas o en ruta</div>
        </div>
        <div style={{ background: "var(--bg-panel)", border: "1px solid var(--line-1)", borderRadius: "var(--r-md)", padding: "16px 18px" }}>
          <div style={{ fontSize: 11.5, color: "var(--ink-3)", fontWeight: 500, marginBottom: 6 }}>Alertas de stock</div>
          ...
          <div style={{ fontSize: 11.5, color: "var(--ink-3)", marginTop: 4 }}>productos bajo mínimo</div>
        </div>
      </div>

      {/* Tab bar + título */}
      <div style={{ display: "flex", alignItems: "center", gap: 0, borderBottom: "1px solid var(--line-1)", marginBottom: 16, flexShrink: 0 }}>
        <span style={{ fontSize: 17, fontWeight: 700, letterSpacing: "-0.02em", color: "var(--ink-1)", paddingRight: 20, borderRight: "1px solid var(--line-1)", marginRight: 4, lineHeight: 1, alignSelf: "center" }}>Almacén</span>
        {TABS.map(t => (
          <button
            key={t.id}
