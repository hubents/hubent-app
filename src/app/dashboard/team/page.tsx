...
const DEFAULT_ROLE_LABELS: Record<string, string> = {
  owner: "Propietario", admin: "Administrador", planner: "Planner",
  assistant: "Asistente", accountant: "Contable", viewer: "Visualizador",
  vendor: "Proveedor", client: "Cliente",
};
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ fontSize: 10.5, fontWeight: 600, textTransform: "uppercase", letterSpacing: ".08em", color: "var(--ink-4)" }}>Contacto</div>
            {[
              { Icon: RiMailLine, val: member.email },
              { Icon: RiTimeLine, val: member.createdAt ? `En el equipo desde ${fmtDate(member.createdAt)}` : null },
        ...
        <div style={{ padding: "14px 24px", borderTop: "1px solid var(--line-1)", display: "flex", gap: 10 }}>
          {canManage && member.role !== "owner" && (
            ...
            >
              <RiDeleteBinLine size={13} /> Eliminar del equipo
            </button>
          )}
          <button
            onClick={onClose}
...
function InviteModal({
  ...
  const handleEmail = (v: string) => {
    setEmail(v);
    setEmailError(v && !isValidEmail(v) ? "Email inválido" : null);
  };

  const submit = async () => {
    if (!isValidEmail(email)) { setEmailError("Email inválido"); return; }
    setSubmitting(true);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(20,18,12,0.45)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={onClose}>
      <div style={{ width: 440, background: "white", borderRadius: "var(--r-md)", padding: 22, boxShadow: "var(--shadow-3)" }} onClick={(e) => e.stopPropagation()}>
        <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 2 }}>Invitar miembro</div>
        <div style={{ fontSize: 12.5, color: "var(--ink-3)", marginBottom: 16 }}>Envía una invitación por email para unirse a tu equipo.</div>

        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div>
            <label style={{ fontSize: 12, fontWeight: 500, marginBottom: 4, display: "block" }}>Email *</label>
            <Inp type="email" placeholder="email@ejemplo.com" value={email} onChange={(e) => handleEmail(e.target.value)} className={emailError ? "border-red-500" : ""} />
            {emailError && <p style={{ fontSize: 11.5, color: "#EF4444", marginTop: 4 }}>{emailError}</p>}
          </div>
          <div>
            <label style={{ fontSize: 12, fontWeight: 500, marginBottom: 4, display: "block" }}>Rol</label>
        ...
        <div style={{ marginTop: 18, display: "flex", justifyContent: "flex-end", gap: 8 }}>
          <Btn variant="outline" onClick={onClose} disabled={submitting}>Cancelar</Btn>
...
export function TeamPageContent({ rolesPath = "/dashboard/settings/roles" }: { rolesPath?: string }) {
  ...
  const getRoleLabel = (slug: string | null) => {
    if (!slug) return "—";
    return allRoles.find((r) => r.slug === slug)?.name || DEFAULT_ROLE_LABELS[slug] || slug;
  };
  ...
  const handleInvite = async (email: string, role: string) => {
    ...
    if (result.success) {
      toast.success("Invitación enviada", { description: `Se envió una invitación a ${email}` });
      setInviteOpen(false);
    } else {
      if (!handleBillingError(result.error || "")) {
        toast.error("Error al enviar invitación", { description: result.error });
      }
    }
  };
  ...
  const handleCancelInv = async (id: number, email: string) => {
    ...
    if (result.success) toast.success(`Invitación a ${email} cancelada`);
    else toast.error("Error al cancelar");
  };

  const handleResendInv = async (id: number, email: string) => {
    const result = await resendInvitation(id);
    if (result.success) toast.success(`Invitación reenviada a ${email}`);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 0, borderRadius: "var(--r-md)", border: "1px solid var(--line-1)", overflow: "hidden", background: "var(--bg-panel)" }}>
        {[
          { label: "Miembros activos",     val: members.length,                           sub: "en el equipo" },
          { label: "Invitaciones pendientes", val: invitations.length,                    sub: "por aceptar" },
          { label: "Total equipo",          val: members.length + invitations.length,     sub: "incluye pendientes" },
          { label: "Roles distintos",       val: new Set(members.map((m) => m.role).filter(Boolean)).size, sub: "en uso" },
        ].map((k, i) => (
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
            ...
            {canManageTeam && (
              <Btn size="sm" className="gap-1.5" onClick={() => setInviteOpen(true)}>
                <RiUserAddLine size={13} /> Añadir miembro
              </Btn>
            )}
          </div>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid var(--line-1)" }}>
                {["Miembro", "Rol", "En el equipo desde", "Estado", ""].map((h, i) => (
                  ...
                  >
                    <td style={{ padding: "10px 10px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <div>
                          <div style={{ fontSize: 13, fontWeight: 600 }}>{m.name || "Sin nombre"}</div>
                  ...
                  <tr key={`inv-${inv.id}`} style={{ borderBottom: "1px solid var(--line-1)" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11.5, color: "var(--ink-4)" }}>
                            {expires ? `Expira: ${fmtDate(expires)}` : "Sin fecha de expiración"}
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td style={{ padding: "10px 10px", width: 80 }}>
                      <div style={{ display: "flex", gap: 4 }}>
                        <button title="Reenviar" onClick={() => handleResendInv(inv.id, inv.email)} style={{ padding: 5, background: "none", border: "none", cursor: "pointer", color: "var(--ink-3)", borderRadius: 4 }}>
                          <RiRefreshLine size={14} />
                        </button>
                        <button title="Cancelar" onClick={() => handleCancelInv(inv.id, inv.email)} style={{ padding: 5, background: "none", border: "none", cursor: "pointer", color: "#EF4444", borderRadius: 4 }}>
                          <RiCloseLine size={14} />
                        </button>
                      </div>
                    </td>
              ...
              {filteredMembers.length === 0 && filteredInvitations.length === 0 && (
                <tr>
                  <td colSpan={5} style={{ textAlign: "center", padding: "40px 10px", color: "var(--ink-3)", fontSize: 13 }}>
                    {query ? "Sin miembros que coincidan" : "No hay miembros en el equipo"}
                  </td>
                </tr>
              )}
            </tbody>
