...
function UploadModal({ folderId, folders, onClose, onUpload }: { folderId: number | null; folders: DocFolder[]; onClose: () => void; onUpload: (doc: Partial<OrgDoc>) => Promise<void> }) {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (!presignData.success) throw new Error(presignData.error?.message || "Error al preparar la subida");
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(20,18,12,0.35)", zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width: 440, background: "var(--bg-panel)", borderRadius: 16, padding: "22px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <div style={{ flex: 1, fontSize: 17, fontWeight: 600, letterSpacing: "-0.01em" }}>Añadir documento</div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {([["upload", "⬆️ Subir archivo"], ["link", "🔗 Enlace externo"]] as const).map(([m, lbl]) => (
        ...
        {mode === "upload" ? (
          <>
            ...
            >
              <IcoUpload size={28} />
              <div style={{ fontSize: 14, fontWeight: 500, color: "var(--ink-2)" }}>Haz clic o arrastra un archivo</div>
              <div style={{ fontSize: 11.5, color: "var(--ink-4)" }}>PDF, Word, Excel, imágenes, ZIP... hasta 50 MB</div>
            </div>
          ...
          <>
            ...
            <div className="form-field">
              ...
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Contrato tipo — Boda.pdf" />
            </div>
            <div className="form-field">
              <label>Tipo</label>
        ...
        {mode === "upload" && saving && (
          <div style={{ textAlign: "center", fontSize: 13, color: "var(--ink-3)" }}>Subiendo archivo...</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(20,18,12,0.35)", zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width: 360, background: "var(--bg-panel)", borderRadius: 14, padding: "20px 22px", display: "flex", flexDirection: "column", gap: 14 }}>
        <div style={{ display: "flex", alignItems: "center" }}>
          <div style={{ flex: 1, fontSize: 16, fontWeight: 600 }}>{initial ? "Editar carpeta" : "Nueva carpeta"}</div>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--ink-3)", padding: 4, borderRadius: 6, display: "flex" }}><IcoX /></button>
        </div>

        <div className="form-field">
          <label>Nombre *</label>
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Ej. Contratos" autoFocus onKeyDown={e => e.key === "Enter" && handleSave()} />
        </div>
  ...
  return (
    <div style={{ padding: "24px 28px", maxWidth: 1280, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-0.02em", margin: 0 }}>Documentos</h1>
          <p style={{ fontSize: 13, color: "var(--ink-3)", marginTop: 4, marginBottom: 0 }}>
            Contratos, propuestas, archivos internos y recursos
          </p>
        </div>
        {activeTab === "archivos" && (
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => { setEditFolder(null); setFolderModalOpen(true); }} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 12px", background: "var(--bg-panel)", color: "var(--ink-2)", border: "1px solid var(--line-1)", borderRadius: "var(--r-sm)", fontSize: 13, cursor: "pointer" }}>
              <IcoFolder size={14} /> Nueva carpeta
            </button>
        ...
        {activeTab === "plantillas" && (
          <button style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 14px", background: "var(--ink-1)", color: "white", border: "none", borderRadius: "var(--r-sm)", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
            <IcoPlus size={13} /> Nueva plantilla
          </button>
        )}
        {activeTab === "firma" && (
          <button style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 14px", background: "var(--ink-1)", color: "white", border: "none", borderRadius: "var(--r-sm)", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
            <IcoSent size={13} /> Enviar para firma
          </button>
        )}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 4, background: "var(--bg-subtle)", borderRadius: "var(--r-sm)", padding: 3, flexShrink: 0 }}>
          {([
            { id: "archivos",   label: "Archivos",         icon: <IcoFolder size={12} />,   n: docs.length },
            { id: "plantillas", label: "Plantillas",        icon: <IcoTemplate size={12} />, n: 6 },
            { id: "firma",      label: "Firma pendiente",   icon: <IcoSign size={12} />,     n: 2 },
          ] as const).map(t => (
            <button
              key={t.id}
              onClick={() => { setActiveTab(t.id); setActiveFolderId(null); setQ(""); }}
              style={{
                display: "inline-flex", alignItems: "center", gap: 6,
                padding: "7px 13px", borderRadius: 6, border: "none", cursor: "pointer",
                fontSize: 12.5, fontWeight: activeTab === t.id ? 600 : 500,
            ...
            >
              {t.icon}
              {t.label}
              <span style={{ fontSize: 11, color: activeTab === t.id ? "var(--ink-3)" : "var(--ink-4)", background: activeTab === t.id ? "var(--bg-subtle)" : "transparent", padding: "1px 6px", borderRadius: 999 }}>
                {t.n}
              </span>
            </button>
          ))}
        ...
        {activeTab === "archivos" && (
          <div style={{ position: "relative", flex: 1, maxWidth: 280 }}>
            ...
            <input value={q} onChange={e => setQ(e.target.value)} placeholder="Buscar documentos..." style={{ width: "100%", paddingLeft: 32, paddingRight: 12, paddingTop: 8, paddingBottom: 8, border: "1px solid var(--line-1)", borderRadius: "var(--r-sm)", background: "var(--bg-panel)", fontSize: 13, color: "var(--ink-1)", outline: "none", boxSizing: "border-box" }} />
          </div>
        )}
      </div>
      ...
      {activeTab === "archivos" && (
        <>
          ...
          {!activeFolderId && !q && (
            <>
              <div style={{ fontSize: 10.5, color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: ".06em", fontWeight: 600, marginBottom: 10 }}>Carpetas</div>
              {folders.length === 0 ? (
                <div style={{ padding: "20px 0", marginBottom: 20, color: "var(--ink-4)", fontSize: 13 }}>
                  Sin carpetas. <button onClick={() => { setEditFolder(null); setFolderModalOpen(true); }} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--ink-2)", textDecoration: "underline", fontSize: 13, padding: 0 }}>Crear una carpeta</button>
                </div>
              ) : (
                  ...
                  {folders.map(f => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: "var(--ink-3)", marginTop: 2 }}>{f.fileCount} {f.fileCount === 1 ? "archivo" : "archivos"}</div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div>
            <div style={{ fontSize: 10.5, color: "var(--ink-3)", textTransform: "uppercase", letterSpacing: ".06em", fontWeight: 600, marginBottom: 10 }}>
              {activeFolder ? `Archivos en ${activeFolder.name}` : q ? "Resultados de búsqueda" : "Recientes"}
            </div>
            {loading ? (
              <div style={{ textAlign: "center", padding: "40px 0", color: "var(--ink-3)" }}>Cargando...</div>
              ...
              <div style={{ textAlign: "center", padding: "40px 0" }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 500, color: "var(--ink-2)", marginBottom: 6 }}>
                  {q ? "Sin resultados" : activeFolder ? "Esta carpeta está vacía" : "Sin documentos"}
                </div>
                {!q && (
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "#FBF2E6", border: "1px solid #E8D9B8", borderRadius: "var(--r-sm)", marginBottom: 18, fontSize: 12, color: "var(--ink-2)" }}>
        <IcoTemplate size={13} />
        <span>Plantillas reutilizables. Duplica, personaliza y envía a tus clientes directamente desde aquí.</span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14 }}>
        {TEMPLATES_DATA.map(t => (
          <div
            ...
            onMouseEnter={e => { e.currentTarget.style.borderColor = t.color; e.currentTarget.style.boxShadow = "0 2px 8px rgba(0,0,0,0.06)"; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = "var(--line-1)"; e.currentTarget.style.boxShadow = "none"; }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ width: 40, height: 40, background: t.color + "22", color: t.color, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <IcoTemplate size={18} />
              </div>
              <span style={{ fontSize: 10.5, color: "var(--ink-3)", background: "var(--bg-subtle)", padding: "2px 8px", borderRadius: 999 }}>{t.category}</span>
            </div>
            <div>
              <div style={{ fontSize: 13.5, fontWeight: 600, color: "var(--ink-1)" }}>{t.name}</div>
              <div style={{ fontSize: 11, color: "var(--ink-3)", marginTop: 3 }}>Usada {t.uses} veces</div>
            </div>
            <div style={{ display: "flex", gap: 6, marginTop: "auto" }}>
              <button style={{ flex: 1, background: t.color, color: "white", border: "none", padding: "7px", borderRadius: "var(--r-sm)", fontSize: 12, fontWeight: 500, cursor: "pointer" }}>Usar plantilla</button>
              <button style={{ background: "white", color: "var(--ink-2)", border: "1px solid var(--line-strong)", padding: "7px 10px", borderRadius: "var(--r-sm)", fontSize: 12, cursor: "pointer" }}>Vista previa</button>
            </div>
          </div>
        ))}
      </div>
...
const STATUS_STYLE = {
  completed: { bg: "#E4EEEA", fg: "#3F6B4D", label: "Firmado" },
  pending:   { bg: "#F4EDE0", fg: "#8A6F3A", label: "Pendiente" },
  expired:   { bg: "#FCE4E0", fg: "#B54632", label: "Caducado" },
};
...
function FirmaView() {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "#FBF2E6", border: "1px solid #E8D9B8", borderRadius: "var(--r-sm)", marginBottom: 18, fontSize: 12, color: "var(--ink-2)" }}>
        <IcoClock size={13} />
        <span>Seguimiento de documentos enviados para firma digital. Los firmantes reciben un email con enlace seguro.</span>
      </div>

      {/* KPI row */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 22 }}>
        {[
          { n: pending,   label: "Pendientes", color: "#8A6F3A", bg: "#FBF2E6" },
          { n: completed, label: "Firmados",   color: "#3F6B4D", bg: "#E4EEEA" },
          { n: expired,   label: "Caducados",  color: "#B54632", bg: "#FCE4E0" },
        ].map((k, i) => (
