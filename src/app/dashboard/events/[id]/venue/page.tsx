    ...
    return (
      <EventSectionGuard eventId={eventId} section="venue">
        <div style={{ padding: "60px 0", textAlign: "center", color: "var(--ink-3)" }}>
          <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 8 }}>Sin venue asignado</div>
          <div style={{ fontSize: 13 }}>Este evento aún no tiene un venue vinculado.</div>
        </div>
      </EventSectionGuard>
    );
  }
    ...
    <EventSectionGuard eventId={eventId} section="venue">
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
          {[
            { label: "Espacios del venue", val: spaces.length, sub: "disponibles" },
            { label: "Otros eventos este mes", val: bookedDays, sub: "días ocupados" },
            { label: "Fines de semana libres", val: freeWeekendDays, sub: `en ${MONTH_NAMES[calMonth]}` },
            { label: "Espacios en tu evento", val: currentDayCount, sub: "reservas tuyas" },
          ].map((k, i) => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: selectedDay != null ? "1fr 300px" : "1fr", gap: 14 }}>
          ...
          <div style={{ padding: 20, borderRadius: "var(--r-md)", border: "1px solid var(--line-1)", background: "var(--bg-panel)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
              <span style={{ fontSize: 11.5, color: "var(--ink-3)", fontWeight: 500 }}>Espacio:</span>
              {[{ id: "all", name: "Todos", color: "var(--ink-2)" }, ...spaces.map((s) => ({ id: String(s.id), name: s.name, color: s.color || "#888" }))].map((s) => (
          ...
          {selectedDay != null && (() => {
            ...
            return (
              <div style={{ padding: 18, borderRadius: "var(--r-md)", border: "1px solid var(--line-1)", background: "var(--bg-panel)", display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "var(--ink-3)", marginTop: 2 }}>
                      {booked.length} reserva{booked.length !== 1 ? "s" : ""} · {free.length} espacio{free.length !== 1 ? "s" : ""} libre{free.length !== 1 ? "s" : ""}
                    </div>
                  </div>
                  <button
                ...
                {booked.length > 0 && (
                  <div>
                    <div style={{ fontSize: 10.5, fontWeight: 600, textTransform: "uppercase", letterSpacing: ".08em", color: "var(--ink-4)", marginBottom: 8 }}>Reservado</div>
                        ...
                        return (
                          <div key={b.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: "var(--r-sm)", background: sc.bg, border: `1px solid ${sc.dot}22` }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontSize: 12.5, fontWeight: 600, color: sc.fg, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.eventName || "Sin nombre"}</div>
                ...
                {free.length > 0 && (
                  <div>
                    <div style={{ fontSize: 10.5, fontWeight: 600, textTransform: "uppercase", letterSpacing: ".08em", color: "var(--ink-4)", marginBottom: 8 }}>Disponibles</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      {free.map((s) => (
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            {(s.capacity || s.type) && (
                              <div style={{ fontSize: 11, color: "#1F6A3A", opacity: 0.75 }}>
                                {[s.capacity ? `${s.capacity} personas` : null, s.type].filter(Boolean).join(" · ")}
                              </div>
                            )}
                          </div>
                ...
                {booked.length === 0 && free.length === 0 && (
                  <div style={{ textAlign: "center", color: "var(--ink-4)", fontSize: 12.5, padding: "20px 0" }}>Sin datos para este día</div>
                )}
              </div>
            );
          })()}
