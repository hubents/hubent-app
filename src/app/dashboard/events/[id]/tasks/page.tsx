...
export default function EventTasksPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fetch event data to set active event
  useEffect(() => {
          ...
          {columns.map((column) => {
            ...
            return (
              <div key={column.id} className="flex flex-col">
                <div className={cn("rounded-t-lg px-4 py-3 font-medium flex items-center justify-between", column.color)}>
                  <span>{column.title}</span>
                  <Badge variant="secondary" className="ml-2">
                    {columnTasks.length}
                  </Badge>
                </div>
                <div className="flex-1 bg-[var(--muted)]/30 rounded-b-lg p-2 min-h-[400px] space-y-2">
                    ...
                    >
                      <CardContent className="p-3">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <RiDraggable className="h-4 w-4 text-[var(--muted-foreground)] flex-shrink-0" />
                        </div>
                        ...
                        {task.category && (
                          <span className="inline-block mt-2 px-2 py-0.5 rounded text-xs bg-[var(--accent)] text-[var(--accent-foreground)]">
                            {task.category}
                          </span>
                        )}
                      </CardContent>
            ...
            <div className="divide-y">
              {filteredTasks.length > 0 ? (
                filteredTasks.map((task) => (
                  <div
                    key={task.id}
                    className="flex items-center justify-between p-4 hover:bg-[var(--muted)]/50 cursor-pointer transition-colors"
                    onClick={() => handleTaskClick(task)}
                  >
                    <div className="flex-1 min-w-0">
                      <h4 className={cn("font-medium", task.status === "completed" && "line-through text-[var(--muted-foreground)]")}>
                        {task.title}
                      </h4>
