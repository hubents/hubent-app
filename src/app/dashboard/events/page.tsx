...
// When the user picked "Otro" and typed a custom label, show that label
// instead of the generic "Otro". Falls back to the standard label otherwise.
...
// Filter options shown in the popover (label kept in Spanish, value = type key).
...
const formatDateProto = (dateStr: string | null) => {
...
const SORT_GROUPS: Array<{ group: string; items: Array<{ v: SortOption; l: string }> }> = [
  { group: "Fecha",       items: [{ v: "date_asc", l: "Fecha más cercana" }, { v: "date_desc", l: "Fecha más lejana" }] },
  { group: "Nombre",      items: [{ v: "name_asc", l: "A → Z" }, { v: "name_desc", l: "Z → A" }] },
  { group: "Presupuesto", items: [{ v: "budget_desc", l: "Mayor a menor" }, { v: "budget_asc", l: "Menor a mayor" }] },
  { group: "Otros",       items: [{ v: "guests", l: "Invitados (↓)" }, { v: "progress", l: "Progreso (↓)" }] },
];
...
export default function EventsPage() {
  ...
  const handleDelete = async (event: Event) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`«${event.name}» eliminado`);
        loadEvents();
      } else {
  ...
  const filterButtonLabel =
    filterType ? `Tipo: ${typeLabels[filterType] || filterType}` : "Filtrar";
    ...
    >
      ...
      {isPending && (
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex gap-1">
            ...
            >
              <IcoCheck className="h-3 w-3" /> Aceptar
            </button>
            <button
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
          ...
          {event.status && STATUS_META[event.status] && (
            <span
              style={{
                background: STATUS_META[event.status].bg,
                color: STATUS_META[event.status].ink,
                borderRadius: 999,
                fontSize: 10,
                fontWeight: 600,
                padding: "2px 8px",
      ...
      >
        <CardField label="Fecha del evento" value={formatDateProto(event.date)} />
        <CardField label="Lugar" value={event.location || "Por definir"} />
      </div>

      {/* Footer: Encargado | Invitados */}
...
// ============================================================
// EventsTable — prototype columns: Nombre / Presupuesto / Invitados /
// Lugar / Progreso / Participantes / Tipo
// ============================================================
      ...
      <table className="w-full" style={{ borderCollapse: "collapse" }}>
        <thead>
          <tr style={{ borderBottom: "1px solid var(--line-1)" }}>
            {["Nombre del evento", "Presupuesto", "Invitados", "Lugar", "Progreso", "Participantes", "Tipo"].map((h) => (
              <th
                key={h}
