...
const TASK_STATUS_STYLE: Record<string, { bg: string; fg: string; label: string }> = {
  pending:     { bg: "#E8EEF6", fg: "#4A6A94", label: "Pendiente" },
  in_progress: { bg: "#FEF3CD", fg: "#8A6D00", label: "En curso" },
  done:        { bg: "#D9ECD1", fg: "#1F6A3A", label: "Completada" },
  cancelled:   { bg: "#FBEDEC", fg: "#B55450", label: "Cancelada" },
};
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: 440, background: "var(--bg-panel)", borderTopLeftRadius: 16, borderBottomLeftRadius: 16, padding: "22px 24px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", marginBottom: 4 }}>
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 17, fontWeight: 600, letterSpacing: "-0.01em" }}>{editing ? "Editar servicio" : "Nuevo servicio"}</div>
              <div style={{ fontSize: 12, color: "var(--ink-3)", marginTop: 1 }}>Catálogo de servicios AV</div>
            </div>
          </div>
        ...
        <div className="form-field"><label>Nombre *</label><input value={form.name} onChange={e => set("name", e.target.value)} placeholder="Ej. Reportaje fotográfico boda full-day" /></div>
        ...
        <div className="form-field">
          ...
          <textarea value={form.description} onChange={e => set("description", e.target.value)} placeholder="Detalle del servicio, incluye, duración..." style={{ resize: "vertical", minHeight: 70 }} />
        </div>
        ...
        <SectionDivider label="Color del avatar" />
...
function ServicesTab() {
  ...
  if (loading) return <div style={{ textAlign: "center", padding: "60px 0", color: "var(--ink-3)" }}>Cargando servicios...</div>;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        ...
        <div style={{ position: "relative" }}>
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Buscar servicios..." style={{ paddingLeft: 30, paddingRight: 10, paddingTop: 7, paddingBottom: 7, border: "1px solid var(--line-1)", borderRadius: "var(--r-sm)", background: "var(--bg-panel)", fontSize: 12.5, width: 180, color: "var(--ink-1)", outline: "none" }} />
        </div>
        ...
        <button onClick={() => { setEditItem(null); setDrawerOpen(true); }} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 14px", background: "var(--ink-1)", color: "white", border: "none", borderRadius: "var(--r-sm)", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
          <IcoPlus size={13} /> Nuevo servicio
        </button>
      </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 0" }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 500, color: "var(--ink-2)", marginBottom: 6 }}>
            {services.length === 0 ? "Sin servicios todavía" : "No hay servicios que coincidan"}
          </div>
          {services.length === 0 && (
            <div style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 16 }}>Crea tu catálogo de servicios: reportajes, vídeo, DJ, drones...</div>
          )}
          {services.length === 0 && (
            <button onClick={() => setDrawerOpen(true)} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 16px", background: "var(--ink-1)", color: "white", border: "none", borderRadius: "var(--r-sm)", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
              <IcoPlus size={13} /> Crear primer servicio
            </button>
          )}
        </div>
      ) : (
              ...
              >
                ...
                <div style={{ padding: 16 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 10, borderTop: "1px solid var(--line-1)" }}>
                    <div>
                      <div style={{ fontSize: 11, color: "var(--ink-3)" }}>Total c/IVA</div>
                      <div style={{ fontSize: 16, fontWeight: 700, color: "var(--ink-1)" }}>{fmt(total)}</div>
                      {s.cost && <div style={{ fontSize: 10.5, color: "var(--ink-4)", marginTop: 1 }}>Coste: {fmt(s.cost)}</div>}
                    </div>
...
function AgendaTab() {
  ...
  if (loading) return <div style={{ textAlign: "center", padding: "60px 0", color: "var(--ink-3)" }}>Cargando agenda...</div>;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", gap: 2, background: "var(--bg-subtle)", borderRadius: "var(--r-sm)", padding: 3, border: "1px solid var(--line-1)", width: "fit-content" }}>
        {STATUS_TABS.map(t => (
          <button key={t.id} onClick={() => setStatusFilter(t.id)} style={{ padding: "5px 12px", borderRadius: "var(--r-sm)", border: "none", cursor: "pointer", background: statusFilter === t.id ? "var(--bg-panel)" : "transparent", boxShadow: statusFilter === t.id ? "var(--shadow-1)" : "none", color: statusFilter === t.id ? "var(--ink-1)" : "var(--ink-3)", fontSize: 12.5, fontWeight: statusFilter === t.id ? 600 : 400 }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 0" }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 500, color: "var(--ink-2)", marginBottom: 6 }}>
            {tasks.length === 0 ? "Sin proyectos en la agenda" : "Sin proyectos con este estado"}
          </div>
          <div style={{ fontSize: 12, color: "var(--ink-3)" }}>
            Cuando un planner te invite a un evento, tus tareas aparecerán aquí
          </div>
        </div>
      ) : (
...
function DeliveriesTab() {
  ...
  const handleCreate = () => {
    ...
    setCreateOpen(false);
    toast.success("Enlace de entrega creado");
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ background: "#EEF6FF", border: "1px solid #BFDBFE", borderRadius: "var(--r-sm)", padding: "12px 16px", display: "flex", gap: 10, alignItems: "flex-start" }}>
        ...
        <div style={{ fontSize: 12.5, color: "#3A5B8A", lineHeight: 1.55 }}>
          <strong>Entregas a clientes:</strong> Crea enlaces para que tus clientes puedan ver y descargar su galería o vídeo.
          Los enlaces apuntan a tu servicio de almacenamiento (Google Drive, Dropbox, Pixieset, WeTransfer, etc.)
        </div>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <button onClick={() => setCreateOpen(true)} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 14px", background: "var(--ink-1)", color: "white", border: "none", borderRadius: "var(--r-sm)", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
          <IcoPlus size={13} /> Nuevo enlace de entrega
        </button>
      </div>
      ...
      {deliveries.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 0" }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 500, color: "var(--ink-2)", marginBottom: 6 }}>Sin entregas todavía</div>
          <div style={{ fontSize: 12, color: "var(--ink-3)" }}>Crea tu primer enlace de entrega para un cliente</div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {deliveries.map(dl => {
                  ...
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    ...
                    {dl.expiresAt && (
                      <span style={{ fontSize: 12, color: isExpired ? "#B55450" : "var(--ink-3)" }}>
                        {isExpired ? "⚠️ Caducado" : `Expira: ${new Date(dl.expiresAt).toLocaleDateString("es-ES")}`}
                      </span>
                    )}
                  </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(20,18,12,0.35)", zIndex: 60, display: "flex", justifyContent: "flex-end" }} onClick={() => setCreateOpen(false)}>
          <div onClick={e => e.stopPropagation()} style={{ width: 420, background: "var(--bg-panel)", borderTopLeftRadius: 16, borderBottomLeftRadius: 16, padding: "22px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "flex", alignItems: "center" }}>
              <div style={{ flex: 1, fontSize: 17, fontWeight: 600, letterSpacing: "-0.01em" }}>Nuevo enlace de entrega</div>
            ...
            <div className="form-field">
              ...
              <input value={form.clientName} onChange={e => setForm(f => ({ ...f, clientName: e.target.value }))} placeholder="Ej. Cami y Juanca" autoFocus />
            </div>
            <div className="form-field">
              <label>Evento</label>
              <input value={form.eventName} onChange={e => setForm(f => ({ ...f, eventName: e.target.value }))} placeholder="Ej. Boda — 17 mayo 2026" />
            </div>
            <div className="form-field">
              <label>URL de la galería / entrega *</label>
              <input type="url" value={form.url} onChange={e => setForm(f => ({ ...f, url: e.target.value }))} placeholder="https://galeria.pixieset.com/..." />
              <div style={{ fontSize: 11, color: "var(--ink-4)", marginTop: 4 }}>Drive, Dropbox, Pixieset, WeTransfer, Vimeo, YouTube (privado)...</div>
            </div>
            <div className="form-field">
              <label>Fecha de caducidad <span style={{ color: "var(--ink-4)", fontWeight: 400 }}>(opcional)</span></label>
              <input type="date" value={form.expiresAt} onChange={e => setForm(f => ({ ...f, expiresAt: e.target.value }))} />
            </div>

            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", paddingTop: 12, borderTop: "1px solid var(--line-1)" }}>
              <button onClick={() => setCreateOpen(false)} style={{ padding: "8px 16px", borderRadius: "var(--r-sm)", border: "1px solid var(--line-1)", background: "var(--bg-panel)", color: "var(--ink-2)", fontSize: 13, cursor: "pointer" }}>Cancelar</button>
...
export default function AudiovisualPage() {
  ...
  const TABS: { id: Tab; label: string; icon: React.ReactNode; desc: string }[] = [
    { id: "services",   label: "Servicios",  icon: <IcoCam size={16} />,   desc: "Catálogo de paquetes y precios" },
    { id: "agenda",     label: "Agenda",     icon: <IcoCal size={16} />,   desc: "Proyectos y eventos pendientes" },
    { id: "deliveries", label: "Entregas",   icon: <IcoShare size={16} />, desc: "Galerías y vídeos para clientes" },
  ];
  ...
  return (
    <div style={{ padding: "24px 28px", maxWidth: 1280, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-0.02em", margin: 0 }}>Audiovisual</h1>
        <p style={{ fontSize: 13, color: "var(--ink-3)", marginTop: 4, marginBottom: 0 }}>
          Gestión de servicios, proyectos y entregas a clientes
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 24, maxWidth: 560 }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 6, padding: "14px 16px", borderRadius: 12, border: `1.5px solid ${activeTab === t.id ? "var(--ink-1)" : "var(--line-1)"}`, background: activeTab === t.id ? "var(--bg-subtle)" : "var(--bg-panel)", cursor: "pointer", textAlign: "left", transition: "all .12s" }}>
            <div style={{ color: activeTab === t.id ? "var(--ink-1)" : "var(--ink-3)" }}>{t.icon}</div>
            <div style={{ fontSize: 13, fontWeight: activeTab === t.id ? 700 : 500, color: activeTab === t.id ? "var(--ink-1)" : "var(--ink-2)" }}>{t.label}</div>
            <div style={{ fontSize: 11, color: "var(--ink-4)", lineHeight: 1.3 }}>{t.desc}</div>
          </button>
        ))}
      </div>
