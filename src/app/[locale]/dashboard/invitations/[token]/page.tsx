...
export default function InvitationPage({ params }: { params: Promise<{ token: string }> }) {
    ...
    async function loadInvitation() {
      try {
        ...
        if (data.success) {
          ...
          setError(data.error?.message || t("invitationNotFound"));
        }
      } catch {
  ...
  async function handleAction(action: "accept" | "reject") {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(action === "accept" ? t("invitationAccepted") : t("invitationRejected"));
        if (action === "accept") {
        ...
        toast.error(data.error?.message || t("errorProcessingInvitation"));
      }
    } catch {
  ...
  const permissionLabels: Record<string, string> = {
    general: t("permGeneral"), calendar: t("permCalendar"), tasks: t("permTasks"),
    partners: t("permPartners"), finances: t("permFinances"), rsvp: t("permRsvp"),
    guests: t("permGuests"), runsheet: t("permRunsheet"),
  };
    ...
    <div className="flex items-center justify-center min-h-[60vh]">
      <PCard className="w-full max-w-md">
        <div style={{ textAlign: "center", marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 600, color: "var(--ink-1)" }}>{t("invitationToCollaborate")}</div>
          <div style={{ fontSize: 13, color: "var(--ink-3)", marginTop: 4 }}>
            <strong>{invitation.hostOrgName}</strong> {t("invitesYouToCollaborate")}
          </div>
        </div>
        <div className="space-y-4">
          ...
          {invitation.permissions && (
            <div>
              <p className="text-sm font-medium mb-2">{t("accessGranted")}:</p>
          ...
          {!isPending && (
            <div className="text-center py-2">
              ...
              >
                {invitation.status === "active" ? t("accepted") : invitation.status === "rejected" ? t("rejected") : invitation.status}
              </Pill>
            </div>
          )}
