...
function ProformasContent() {
  ...
  const STATUS_PILL: Record<string, { bg: string; fg: string; label: string }> = {
    draft:     { bg: "#EDEAE3", fg: "#5B5649", label: t("proformas.statusDraft") },
    approved:  { bg: "#D4E7F0", fg: "#2F6A85", label: t("proformas.statusApproved") },
    sent:      { bg: "#FCEFC9", fg: "#8A6A1A", label: t("proformas.statusSent") },
    paid:      { bg: "#D9ECD1", fg: "#1F6A3A", label: t("proformas.statusPaid") },
    cancelled: { bg: "#F8D4D4", fg: "#8B2A2A", label: t("proformas.statusCancelled") },
  };

  const STATUS_OPTIONS: { value: StatusFilter; label: string }[] = [
    { value: "all",       label: t("filters.statusAllFem") },
    { value: "draft",     label: t("proformas.statusDraft") },
    { value: "approved",  label: t("proformas.statusApproved") },
    { value: "sent",      label: t("proformas.statusSent") },
    { value: "paid",      label: t("proformas.statusPaid") },
    { value: "cancelled", label: t("proformas.statusCancelled") },
  ];
  ...
  async function fetchProformas() {
    try {
      ...
      setFetchError(t("proformas.connectError"));
      toast.error(t("proformas.loadError"));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function deleteProforma(id: number) {
    if (!await appConfirm({ title: t("proformas.deleteTitle"), description: t("proformas.deleteDescription"), confirmLabel: t("proformas.deleteConfirm"), variant: "destructive" })) return;
    try {
      const res = await fetch(`/api/finance/documents/${id}`, { method: "DELETE" });
      if (res.ok) { toast.success(t("proformas.deleted")); fetchProformas(); }
      else { const d = await res.json().catch(() => null); toast.error(d?.error?.message || t("proformas.deleteError")); }
    } catch { toast.error(t("proformas.deleteError")); }
  }
  ...
  async function fetchDocAndOpenDrawer(id: number, targetType: "proforma" | "invoice" | "delivery_note") {
    ...
    } catch { toast.error(t("proformas.loadDocError")); }
  }
  ...
  async function updateStatus(id: number, status: string) {
    try {
      ...
      if (res.ok) {
        toast.success(`${t("proformas.statusUpdated")} ${STATUS_PILL[status]?.label || status}`);
        fetchProformas();
      } else {
        const d = await res.json().catch(() => null);
        toast.error(d?.error?.message || t("proformas.statusError"));
      }
    } catch { toast.error(t("proformas.statusError")); }
  }
  ...
  const kpis = useMemo(() => {
    ...
    return [
      { label: t("proformas.kpiIssued"),    value: formatCurrency(String(total)),           delta: "",                    sub: t("proformas.kpiOfList") },
      { label: t("proformas.kpiPaid"),      value: formatCurrency(String(sum(pagadas))),     delta: pct(sum(pagadas)),     sub: t("proformas.kpiOfTotal") },
      { label: t("proformas.kpiPending"),   value: formatCurrency(String(sum(pendientes))),  delta: pct(sum(pendientes)),  sub: t("proformas.kpiOfTotal") },
      { label: t("proformas.kpiCancelled"), value: formatCurrency(String(sum(cancelled))),   delta: pct(sum(cancelled)),   sub: t("proformas.kpiOfTotal") },
    ];
      ...
      >
        ...
        <div className="flex items-center gap-2.5 mb-3 flex-wrap">
          ...
          >
            ...
            <input
              type="text"
              placeholder={t("proformas.search")}
              value={searchTerm}
          ...
          <button
            ...
            title={selectedIds.size > 0 ? t("proformas.downloadTitle", { count: selectedIds.size }) : t("proformas.downloadHint")}
        ...
        {fetchError ? (
          ...
          <div className="text-center py-14">
            <div className="text-[14px] font-semibold text-[var(--ink-1)] mb-1">{t("proformas.empty")}</div>
            <div className="text-[12.5px] text-[var(--ink-3)] mb-4">
              {searchTerm ? t("proformas.emptySearch") : t("proformas.emptyCreate")}
            </div>
              ...
              <thead>
                <tr>
                  ...
                  <th onClick={() => cycleSort("client")} style={{ cursor: "pointer", userSelect: "none" }}>
                    <span className="inline-flex items-center gap-1">{t("proformas.colClient")} <SortIcon k="client" /></span>
                  </th>
                  <th onClick={() => cycleSort("issueDate")} style={{ cursor: "pointer", userSelect: "none" }}>
                    <span className="inline-flex items-center gap-1">{t("proformas.colDate")} <SortIcon k="issueDate" /></span>
                  </th>
                  <th onClick={() => cycleSort("number")} style={{ cursor: "pointer", userSelect: "none" }}>
                    <span className="inline-flex items-center gap-1">{t("proformas.colNumber")} <SortIcon k="number" /></span>
                  </th>
                  <th>{t("proformas.colPaid")}</th>
                  <th onClick={() => cycleSort("status")} style={{ cursor: "pointer", userSelect: "none" }}>
                    <span className="inline-flex items-center gap-1">{t("proformas.colStatus")} <SortIcon k="status" /></span>
                  </th>
                  <th
                  ...
                  >
                    <span className="inline-flex items-center gap-1 justify-end w-full">
                      <SortIcon k="total" /> {t("proformas.colTotal")}
                    </span>
                  </th>
                  <th style={{ width: 44 }} />
                </tr>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => openPreview(doc.id)}>
                              <IcoEye className="mr-2 h-4 w-4" /> {t("proformas.actionPreview")}
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => openEditDrawer(doc.id)}>
                              <IcoEdit className="mr-2 h-4 w-4" /> {t("proformas.actionEdit")}
                            </DropdownMenuItem>
                            ...
                            {doc.status === "draft" && (
                              <>
                                <DropdownMenuItem onClick={() => updateStatus(doc.id, "approved")}>
                                  <IcoCheckDouble className="mr-2 h-4 w-4" /> {t("proformas.actionApprove")}
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => updateStatus(doc.id, "sent")}>
                                  <IcoSend className="mr-2 h-4 w-4" /> {t("proformas.actionMarkPending")}
                                </DropdownMenuItem>
                              </>
                            )}
                            {doc.status === "approved" && (
