...
const STATUS_PILL_COLORS: Record<string, { bg: string; fg: string }> = {
  paid:      { bg: "#DCEFE1", fg: "#1F6B3A" },
  accepted:  { bg: "#DCEFE1", fg: "#1F6B3A" },
  sent:      { bg: "#E0E7F8", fg: "#2A4A8B" },
  pending:   { bg: "#FBEBC9", fg: "#8B5E1A" },
  partial:   { bg: "#FBEBC9", fg: "#8B5E1A" },
  overdue:   { bg: "#F8D4D4", fg: "#8B2A2A" },
  rejected:  { bg: "#F8D4D4", fg: "#8B2A2A" },
  cancelled: { bg: "#EEEAE0", fg: "#6B6660" },
  draft:     { bg: "#EEEAE0", fg: "#6B6660" },
};
...
export default function FinanceDashboardPage() {
  ...
  const shortcuts: Array<{
    ...
    {
      Icon: RiFileTextLine,
      title: t("shortcuts.newQuoteTitle"),
      subtitle: t("shortcuts.newQuoteSubtitle"),
      href: "/dashboard/finance/quotes/new",
      show: canCreate,
    },
    {
      Icon: RiFileList2Line,
      title: t("shortcuts.newInvoiceTitle"),
      subtitle: t("shortcuts.newInvoiceSubtitle"),
      href: "/dashboard/finance/invoices/new",
      show: canCreate,
    },
    {
      Icon: RiMoneyDollarCircleLine,
      title: t("shortcuts.newPaymentTitle"),
      subtitle: t("shortcuts.newPaymentSubtitle"),
      href: "/dashboard/finance/payments",
      show: canCreate,
    },
    {
      Icon: RiDownloadLine,
      title: t("shortcuts.exportTitle"),
      subtitle: t("shortcuts.exportSubtitle"),
  ...
  const kpis = [
    ...
    },
    {
      ...
      sub: t("overview.overdueInvoices"),
      pos: false,
      isCount: true,
    },
  ];
              ...
              return (
                ...
                >
                  {isPeak && (
                    ...
                    >
                      <div>{t("overview.incomeTooltip")} {fmt(b.inc, stats?.currency)}</div>
                      <div>{t("overview.expensesTooltip")} -{fmt(b.exp, stats?.currency)}</div>
                    </div>
                  )}
                  <div
                    className="fin-bar"
            ...
            {[
              {
                Icon: RiCalendarLine,
                color: "#6FB3D2",
                label: t("overview.agingCurrent"),
                value: stats?.aging.current || 0,
              },
              {
                Icon: RiAlertLine,
                color: "#E14F4F",
                label: t("overview.agingOverdue"),
                value: stats?.overdueInvoices || 0,
              },
              {
                Icon: RiMoneyDollarCircleLine,
                color: "#A8845C",
                label: t("overview.agingReceivable"),
      ...
      >
              ...
              {stats.topClients.slice(0, 5).map((c, i) => (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      {c.email || `${c.invoiceCount} ${t("overview.invoicesCount")}`}
                    </div>
                  </div>
        ...
        <FinCard style={{ padding: 16 }}>
          ...
          {recent.length === 0 ? (
            ...
            >
              {t("overview.noRecentDocuments")}
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {recent.slice(0, 4).map((d) => {
                const pillColors = STATUS_PILL_COLORS[d.status] || {
                  bg: "#EEEAE0",
                  fg: "#6B6660",
                };
                ...
                const typeKey = typeKeyMap[d.type];
                const typeLabel = typeKey ? t(`types.${typeKey}`) : d.type;
                ...
                const statusLabel = knownStatuses.includes(statusKey)
                  ? t(`status.${statusKey as "paid" | "pending" | "draft" | "overdue" | "sent" | "accepted" | "cancelled" | "rejected" | "partial" | "delivered" | "payment_promise" | "approved" | "complete"}`)
