    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--muted-foreground)]">
            {vendors.length} {t("assignedPartners")}
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Link href="/dashboard/partners">
            <Btn variant="outline">{t("partners")}</Btn>
          </Link>
        </div>
      </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {platformProviders.map((pp) => {
              const statusMap: Record<string, { label: string; bg: string; color: string }> = {
                active: { label: t("statusActive"), bg: "#DCFCE7", color: "#166534" },
                pending: { label: t("statusPending"), bg: "#FEF9C3", color: "#854D0E" },
                rejected: { label: t("statusRejected"), bg: "#FEE2E2", color: "#991B1B" },
                revoked: { label: t("statusRevoked"), bg: "var(--bg-subtle)", color: "var(--ink-2)" },
              };
      ...
      <Sheet open={showInviteDrawer} onOpenChange={setShowInviteDrawer}>
        <SheetContent className="sm:max-w-2xl overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{t("invitePartnerToEvent")}</SheetTitle>
          </SheetHeader>
          <div className="space-y-4 px-4 pb-4">
            <p className="text-sm text-muted-foreground">
              {t("searchPartnersDesc")}
            </p>
            <div className="relative">
              ...
              <Inp
                ...
                placeholder={t("searchInPartners")}
                className="pl-10"
              />
            </div>
                ...
                {directoryProviders.map((dp) => {
                  ...
                  return (
                    <div key={dp.id} className="flex items-center justify-between p-3 rounded-lg border">
                      ...
                      {alreadyInvited ? (
                        <Pill bg="var(--bg-subtle)" color="var(--ink-2)">{t("invited")}</Pill>
                      ) : (
                        <Btn
                          size="sm"
      ...
      {filteredVendors.length > 0 ? (
        ...
        <PCard style={{ textAlign: "center", padding: "48px 24px" }}>
          ...
          <h3 className="font-semibold mb-2">{t("noPartnersAssigned")}</h3>
          <p className="text-[var(--muted-foreground)] mb-4">
            {t("invitePartnersDesc")}
          </p>
          {canEditVendors && (
          <Btn onClick={() => setShowInviteDrawer(true)}>
            <RiAddLine className="h-4 w-4 mr-2" />
            {t("inviteFirstPartner")}
          </Btn>
          )}
        </PCard>
      )}
