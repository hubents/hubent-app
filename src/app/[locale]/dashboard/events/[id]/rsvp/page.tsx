...
export default function EventRsvpPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [settings, setSettings] = useState<RsvpSettings>({
    ...
    showFaqs: true,
    showTransport: false,
    menuOptions: [t("rsvpDefaultMenuMeat"), t("rsvpDefaultMenuFish"), t("rsvpDefaultMenuVeg")],
  });
    ...
    onSuccess: async (result) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(data?.error || t("rsvpCoverSaveError"));
        }
        toast.success(t("rsvpCoverSaved"));
      } catch (err) {
        const msg = err instanceof Error ? err.message : t("rsvpCoverSaveErrorGeneric");
        toast.error(msg);
      }
    },
    onError: (msg) => {
      ...
      if (msg.includes("almacenamiento") || msg.includes("CONFIG")) {
        toast.error(t("rsvpStorageNotConfigured"));
      } else {
        toast.error(msg);
      }
    },
            ...
            const next: RsvpSettings = {
              ...
              menuOptions:
                Array.isArray(s.menuOptions) && s.menuOptions.length > 0
                  ? s.menuOptions
                  : [t("rsvpDefaultMenuMeat"), t("rsvpDefaultMenuFish"), t("rsvpDefaultMenuVeg")],
            };
            setSettings(next);
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      toast.success(t("rsvpLinkCopied"));
    } catch {
      toast.error(t("rsvpLinkCopyError"));
    }
  };
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(t("rsvpSaveSuccess"));
        setHasChanges(false);
        initialSettingsRef.current = JSON.stringify(settings);
      } else {
        toast.error(t("rsvpSaveError"));
      }
    } catch (error) {
      console.error("Failed to save settings:", error);
      toast.error(t("rsvpSaveConnError"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveCoverUrl = async (url: string | null) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t("rsvpCoverSaveError"));
      toast.success(url ? t("rsvpCoverSaved") : t("rsvpCoverRemoved"));
    } catch (err) {
  ...
  const saveBus = async (item: Partial<TransportOption>) => {
    if (editingBus) {
      ...
      const data = await res.json();
      if (data.success) {
        setTransportOptions((prev) => prev.map((tp) => (tp.id === merged.id ? merged : tp)));
      }
    } else {
      const res = await fetch(`/api/events/${eventId}/rsvp/transport`, {
        method: "POST",
      ...
      <div className="flex flex-col gap-3.5">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          >
            {(["editor", "preview"] as const).map((k) => {
              ...
              return (
                ...
                >
                  {k === "editor" ? t("rsvpTabEditor") : t("rsvpTabPreview")}
                </button>
              );
            })}
          </div>
          ...
          <div className="ml-auto flex items-center gap-2 flex-wrap">
            ...
            >
              <IcoSend className="h-3 w-3" />
              {saving ? t("rsvpPublishing") : hasChanges ? t("rsvpPublishPending") : t("rsvpPublish")}
            </button>
          </div>
        </div>
          ...
          >
            ...
            <div className="flex flex-col gap-3.5">
              ...
              <div className="grid grid-cols-4 gap-2.5">
                <RsvpKpi label={t("rsvpKpiTotal")} value={rsvpStats.total} tone="ink" />
                <RsvpKpi label={t("rsvpKpiConfirmed")} value={rsvpStats.confirmed} tone="success" />
                <RsvpKpi label={t("rsvpKpiPending")} value={rsvpStats.pending} tone="warn" />
                <RsvpKpi label={t("rsvpKpiRejected")} value={rsvpStats.declined} tone="danger" />
              </div>

              {/* Portada */}
              <RsvpBlock title={t("rsvpCoverTitle")} icon={<IcoImage className="h-3.5 w-3.5" />}>
                  ...
                  <div className="flex flex-col gap-2 justify-center">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      >
                        ...
                        {uploadingImage ? t("rsvpUploading") : t("rsvpUploadImage")}
                      </button>
                      {coverImage && ed && (
                        <button
                    ...
                    <input
                      type="url"
                      placeholder={t("rsvpPasteUrlPlaceholder")}
              ...
              {/* Bienvenida */}
              <RsvpBlock title={t("rsvpWelcomeTitle")} icon={<IcoMessage className="h-3.5 w-3.5" />}>
                <FormField label={t("rsvpMainMessage")}>
                  <textarea
                    rows={2}
                    readOnly={!ed}
                ...
                <div className="grid grid-cols-3 gap-2.5 mt-2.5">
                  <FormField label={t("rsvpFieldDate")}>
                    <input
                      type="date"
                      defaultValue={
                        activeEvent?.date
                  ...
                  <FormField label={t("rsvpFieldTime")}>
                    <input
                      type="time"
                      defaultValue={
                        activeEvent?.date
                  ...
                  <FormField label={t("rsvpFieldPlace")}>
              ...
              <RsvpBlock
                title={t("rsvpAttendanceTitle")}
                icon={<IcoCheckCircle className="h-3.5 w-3.5" />}
              >
                <div className="grid grid-cols-2 gap-2.5">
                  <FormField label={t("rsvpDeadlineField")}>
                    <input
                      type="date"
                  ...
                  <FormField label={t("rsvpMaxCompanions")}>
                    <input
                      type="number"
                      min="0"
                      max="10"
              ...
              <RsvpSection
                ...
                items={transportOptions.map((tp) => ({
                  id: tp.id,
                  primary: tp.name,
                  cells: [
                    fmtTime(tp.departureTime) || "",
                    tp.name,
                    tp.departureLocation || "",
                    tp.capacity != null ? `${tp.capacity} ${t("rsvpBusSeats")}` : "",
                  ],
                  raw: tp,
                }))}
              ...
              {/* Tema · paleta */}
              <RsvpBlock title={t("rsvpThemeTitle")} icon={<IcoPalette className="h-3.5 w-3.5" />}>
    ...
    >
      ...
      >
        ...
        {yesNoMode ? (
          ...
          >
            {([
              { v: true, label: t("rsvpToggleYes"), bg: "#4F7A5E", fg: "#FFFFFF" },
              { v: false, label: t("rsvpToggleNo"), bg: "var(--ink-1)", fg: "#FFFFFF" },
            ] as const).map((opt) => {
          ...
          >
            <span className="text-[11px] text-[var(--ink-3)]">
              {enabled ? t("rsvpToggleInclude") : t("rsvpToggleHidden")}
            </span>
            <span
              onClick={(e) => {
                e.stopPropagation();
      ...
      {open && (
        <div style={{ padding: "10px 14px 14px" }}>
          {yesNoMode && !enabled ? (
            ...
            >
              {noMessage || t("rsvpSectionNoMessage")}
            </div>
          ) : items.length === 0 ? (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {onEditItem && (
                            <button
                              ...
                              aria-label={t("rsvpEditAriaLabel")}
                            >
                          ...
                          {onDeleteItem && (
                            <button
                              onClick={async (e) => {
                                e.stopPropagation();
                                if (await appConfirm({ title: t("rsvpDeleteItemTitle"), variant: "destructive", confirmLabel: t("rsvpDeleteItemConfirm") }))
                                  onDeleteItem(it.id);
                              }}
                              className="bg-transparent border-none cursor-pointer p-1 text-[var(--ink-3)] hover:text-[var(--color-danger)]"
                              aria-label={t("rsvpDeleteAriaLabel")}
                            >
...
function MenuOptionsEditor({
  ...
  const add = () => onChange([...options, t("rsvpMenuAddOption")]);
      ...
      {options.map((opt, i) => (
        <div key={i} className="flex items-center gap-2">
          <input
            ...
            placeholder={t("rsvpMenuOptionPlaceholder")}
            style={{
              flex: 1,
              padding: "6px 10px",
              border: "1px solid var(--line-1)",
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Horarios */}
        {showItinerary && (
          <PreviewSection title={t("rsvpPreviewSchedule")} accent={accent}>
        ...
        {/* Dónde dormir */}
        {showHotels && (
          <PreviewSection title={t("rsvpPreviewHotels")} accent={accent}>
            {sortedHotels.map((h) => (
        ...
        {/* Transporte */}
        {showBuses && (
          <PreviewSection title={t("rsvpPreviewTransport")} accent={accent}>
            <div
              style={{
                display: "flex",
            ...
            >
              {activeBuses.map((b) => (
                ...
                >
                  ...
                  >
                    {b.departureLocation
                      ? `${b.name} · ${t("rsvpBusDepartsFrom")} ${b.departureLocation}`
                      : b.name}
                  </div>
                </div>
              ))}
        ...
        {/* Actividades */}
        {showPlans && (
          <PreviewSection title={t("rsvpPreviewActivities")} accent={accent}>
            {sortedPlans.map((p) => (
        ...
        {/* FAQ */}
        {showFaqs && (
          <PreviewSection title={t("rsvpPreviewFaq")} accent={accent}>
            {sortedFaqs.map((f) => (
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {isEditing ? t("rsvpDrawerSaveBtn") : t("rsvpDrawerAddBtn")}
          </button>
        </div>
      </div>
    </div>
...
function ItineraryDrawer({
  ...
  return (
    <DrawerShell
      open={open}
      onClose={onClose}
      title={editing ? t("rsvpItineraryEditTitle") : t("rsvpItineraryAddTitle")}
      subtitle={t("rsvpItinerarySubtitle")}
      onSave={() =>
        onSave({
          title: form.title,
    ...
    >
      <div className="grid gap-3 drawer-form-field">
        <FormField label={t("rsvpItineraryTitle")}>
          <input
            value={form.title}
            onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))}
            placeholder={t("rsvpItineraryTitlePlaceholder")}
          />
        </FormField>
        <div className="grid grid-cols-2 gap-2.5">
          <FormField label={t("rsvpItineraryStartTime")}>
            <input
              type="time"
          ...
          <FormField label={t("rsvpItineraryEndTime")}>
            <input
              type="time"
              value={form.endTime}
        ...
        <FormField label={t("rsvpItineraryLocation")}>
          <input
            value={form.location}
            onChange={(e) => setForm((f) => ({ ...f, location: e.target.value }))}
            placeholder={t("rsvpItineraryLocationPlaceholder")}
          />
        </FormField>
        <FormField label={t("rsvpItineraryDesc")}>
          <textarea
            rows={3}
            value={form.description}
            onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
            placeholder={t("rsvpItineraryDescPlaceholder")}
...
function HotelDrawer({
  ...
  return (
    <DrawerShell
      open={open}
      onClose={onClose}
      title={editing ? t("rsvpHotelEditTitle") : t("rsvpHotelAddTitle")}
      subtitle={t("rsvpHotelSubtitle")}
      onSave={() =>
        onSave({
          name: form.name,
          description: form.description || null,
    ...
    >
      <div className="grid gap-3 drawer-form-field">
        <FormField label={t("rsvpHotelName")}>
          <input
            value={form.name}
            onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
            placeholder={t("rsvpHotelNamePlaceholder")}
          />
        </FormField>
        <FormField label={t("rsvpHotelDesc")}>
          <textarea
            rows={2}
            value={form.description}
            onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
            placeholder={t("rsvpHotelDescPlaceholder")}
            style={{ resize: "vertical" }}
          />
        </FormField>
        <FormField label={t("rsvpHotelAddress")}>
          <input
            value={form.address}
            onChange={(e) => setForm((f) => ({ ...f, address: e.target.value }))}
            placeholder={t("rsvpHotelAddressPlaceholder")}
          />
        </FormField>
        <div className="grid grid-cols-2 gap-2.5">
          <FormField label={t("rsvpHotelPhone")}>
            <input
              value={form.phone}
          ...
          <FormField label={t("rsvpHotelPriceRange")}>
            <input
        ...
        <div className="grid grid-cols-2 gap-2.5">
          <FormField label={t("rsvpHotelWebsite")}>
            <input
              value={form.website}
          ...
          <FormField label={t("rsvpHotelDistance")}>
            <input
              value={form.distance}
              onChange={(e) => setForm((f) => ({ ...f, distance: e.target.value }))}
              placeholder={t("rsvpHotelDistancePlaceholder")}
            />
          </FormField>
        </div>
      </div>
...
function BusDrawer({
  ...
  return (
    <DrawerShell
      open={open}
      onClose={onClose}
      title={editing ? t("rsvpBusEditTitle") : t("rsvpBusAddTitle")}
      subtitle={t("rsvpBusSubtitle")}
      onSave={() =>
        onSave({
          name: form.name || t("rsvpBusDefaultName"),
    ...
    >
      <div className="grid gap-3 drawer-form-field">
        <FormField label={t("rsvpBusNameLabel")}>
          <input
            value={form.name}
            onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
            placeholder={t("rsvpBusNamePlaceholder")}
          />
        </FormField>
        <FormField label={t("rsvpBusDepartureLocation")}>
          <input
        ...
        <FormField label={t("rsvpBusDepartureAddress")}>
          <input
        ...
        <div className="grid grid-cols-2 gap-2.5">
          <FormField label={t("rsvpBusDepartureTime")}>
            <input
              type="time"
          ...
          <FormField label={t("rsvpBusReturnTime")}>
            <input
              type="time"
        ...
        <FormField label={t("rsvpBusCapacity")}>
          <input
            type="number"
            min="1"
            value={form.capacity}
...
function PlanDrawer({
  ...
  return (
    <DrawerShell
      open={open}
      onClose={onClose}
      title={editing ? t("rsvpPlanEditTitle") : t("rsvpPlanAddTitle")}
      subtitle={t("rsvpPlanSubtitle")}
      onSave={() =>
        onSave({
          name: form.name,
          description: form.description || null,
    ...
    >
      <div className="grid gap-3 drawer-form-field">
        <FormField label={t("rsvpPlanName")}>
          <input
            value={form.name}
            onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
            placeholder={t("rsvpPlanNamePlaceholder")}
          />
        </FormField>
        <FormField label={t("rsvpPlanCategory")}>
          <input
            value={form.category}
            onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))}
            placeholder={t("rsvpPlanCategoryPlaceholder")}
          />
        </FormField>
        <FormField label={t("rsvpPlanAddress")}>
          <input
            value={form.address}
            onChange={(e) => setForm((f) => ({ ...f, address: e.target.value }))}
            placeholder={t("rsvpPlanAddressPlaceholder")}
          />
        </FormField>
        <FormField label={t("rsvpPlanWebsite")}>
          <input
            value={form.website}
            onChange={(e) => setForm((f) => ({ ...f, website: e.target.value }))}
            placeholder="https://..."
        ...
        <FormField label={t("rsvpPlanDesc")}>
          <textarea
            rows={2}
            value={form.description}
            onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
            placeholder={t("rsvpPlanDescPlaceholder")}
            style={{ resize: "vertical" }}
          />
        </FormField>
      </div>
...
function FaqDrawer({
  ...
  return (
    <DrawerShell
      open={open}
      onClose={onClose}
      title={editing ? t("rsvpFaqEditTitle") : t("rsvpFaqAddTitle")}
      subtitle={t("rsvpFaqSubtitle")}
      onSave={() =>
        onSave({
    ...
    >
      <div className="grid gap-3 drawer-form-field">
        <FormField label={t("rsvpFaqQuestion")}>
          <input
            value={form.question}
            onChange={(e) => setForm((f) => ({ ...f, question: e.target.value }))}
            placeholder={t("rsvpFaqQuestionPlaceholder")}
          />
        </FormField>
        <FormField label={t("rsvpFaqAnswer")}>
          <textarea
            rows={4}
            value={form.answer}
            onChange={(e) => setForm((f) => ({ ...f, answer: e.target.value }))}
            placeholder={t("rsvpFaqAnswerPlaceholder")}
            style={{ resize: "vertical" }}
          />
        </FormField>
      </div>
