...
export default function EventInvoicesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function deleteDoc(docId: number) {
    if (!await appConfirm({ title: t("deleteInvoice"), description: t("cannotUndo"), variant: "destructive", confirmLabel: t("delete") })) return;
    try {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {invoices.length} {invoices.length !== 1 ? t("invoices") : t("invoice")} {t("ofEvent")}
          </p>
        </div>
        {canEditFinances && (
        <Btn variant="primary" onClick={openNewDoc}>
          <RiAddLine className="mr-2 h-4 w-4" /> {t("newInvoice")}
        </Btn>
        )}
      </div>
      ...
      {invoices.length === 0 ? (
        <PCard>
          <div style={{ padding: "48px 20px", textAlign: "center", color: "var(--ink-3)" }}>
            ...
            <p style={{ fontWeight: 500, color: "var(--ink-1)" }}>{t("noInvoicesYet")}</p>
            <p style={{ fontSize: 13, marginTop: 4 }}>{t("noInvoicesYetDesc")}</p>
          </div>
        </PCard>
      ) : (
        <PCard padding={0}>
                    ...
                    >
                      ...
                      <TableCell>
                        <Pill bg="" color="" className={st.color}>{
                          { draft: t("statusDraft"), sent: t("statusPending"), partial: t("statusPartial"), paid: t("statusPaid"), overdue: t("statusOverdue") }[displayStatus] || st.label
                        }</Pill>
                      </TableCell>
