...
function PartnerCard({ partner, canEdit, onStatusChange }: {
  ...
  const t = useTranslations("eventDetail");
  const localStatusMap: Record<string, { label: string; bg: string; fg: string }> = {
    active:               { label: t("statusConfirmed"),     bg: "#D1FAE5", fg: "#065F46" },
    pending:              { label: t("statusPending"),       bg: "#FEF3CD", fg: "#92600A" },
    pending_registration: { label: t("statusUnregistered"), bg: "#F3F4F6", fg: "#6B7280" },
    rejected:             { label: t("statusRejected"),     bg: "#FEE2E2", fg: "#991B1B" },
    revoked:              { label: t("statusRevoked"),      bg: "#F3F4F6", fg: "#6B7280" },
  };
  const st       = localStatusMap[partner.status] ?? localStatusMap.pending;
      ...
      >
        ...
        <div style={{ padding: "22px 24px 16px", borderBottom: "1px solid var(--line-1)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 16, fontWeight: 600, color: "var(--ink-1)" }}>{t("addPartner")}</div>
              <div style={{ fontSize: 12, color: "var(--ink-3)", marginTop: 1 }}>{t("searchInHubentsDirectory")}</div>
            </div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "4px 24px 24px", display: "flex", flexDirection: "column", gap: 6 }}>
          {loading
            ...
            : providers.length === 0
            ? <p style={{ fontSize: 13, color: "var(--ink-3)", textAlign: "center", paddingTop: 32 }}>{t("noResultsFound")}</p>
            : providers.map((p) => {
                ...
                return (
                  <div key={p.id} style={{
                    ...
                    {already ? (
                      <Pill color="#065F46" bg="#D1FAE5">{t("added")}</Pill>
                    ) : (
