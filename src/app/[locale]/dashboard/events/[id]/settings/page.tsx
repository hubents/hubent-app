...
function EventDataForm({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data.success) { toast.success(t("changesSaved")); onSaved(data.data); }
      else toast.error(data.error?.message || t("errorSaving"));
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { toast.success(t("eventCancelled")); onCancelled(data.data); }
      else toast.error(data.error?.message || t("errorCancelling"));
    } catch { toast.error(t("errorConnection")); }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      else toast.error(data.error?.message || t("errorDeleting"));
    } catch { toast.error(t("errorConnection")); }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ padding: 20, borderRadius: "var(--r-md)", border: "1px solid var(--line-1)", background: "var(--bg-panel)" }}>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 14 }}>{t("eventData")}</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div>
            <FieldLabel>{t("eventName")} *</FieldLabel>
            <input style={inp} value={form.name} onChange={(e) => set("name", e.target.value)} placeholder={t("eventNamePlaceholder")} />
          </div>
          <div>
            <FieldLabel>{t("eventType")}</FieldLabel>
            <select style={inp} value={form.type} onChange={(e) => set("type", e.target.value)}>
              <option value="">{t("selectType")}</option>
              <option value="wedding">{t("typeWedding")}</option>
              <option value="prewedding">{t("typePreWedding")}</option>
              <option value="postwedding">{t("typePostWedding")}</option>
              <option value="corporate">{t("typeCorporate")}</option>
              <option value="birthday">{t("typeBirthday")}</option>
          ...
          <div>
            <FieldLabel>{t("totalBudget")} (€)</FieldLabel>
          ...
          <div style={{ gridColumn: "1/-1" }}>
            ...
            <input style={inp} value={form.location} onChange={(e) => set("location", e.target.value)} placeholder={t("locationPlaceholder")} />
          </div>
          <div style={{ gridColumn: "1/-1" }}>
            <FieldLabel>{t("internalNotes")}</FieldLabel>
            <textarea style={{ ...inp, resize: "vertical" }} value={form.description} onChange={(e) => set("description", e.target.value)} placeholder={t("eventDescPlaceholder")} rows={3} />
          </div>
        </div>
      ...
      <div style={{ padding: 16, borderRadius: "var(--r-md)", border: "1px solid var(--line-1)", background: "var(--bg-panel)" }}>
        <div style={{ fontSize: 12.5, fontWeight: 600, color: "var(--ink-3)", marginBottom: 12 }}>{t("dangerZone")}</div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            <button
              ...
              onClick={async () => {
                const msg = taskCount > 0
                  ? t("cancelEventWithTasks", { count: taskCount })
                  : t("cancelEventConfirm");
                if (await appConfirm({ title: t("cancelEvent"), description: msg, variant: "destructive", confirmLabel: t("cancelEvent") })) handleCancel();
              }}
              disabled={cancelling}
            >
          ...
          <button
            ...
            onClick={async () => {
              const msg = taskCount > 0
                ? t("deleteEventWithTasks", { count: taskCount })
                : t("deleteEventConfirm");
              if (await appConfirm({ title: t("deleteEvent"), description: msg, variant: "destructive", confirmLabel: t("delete") })) handleDelete();
            }}
            disabled={deleting}
          >
...
function CollaboratorsTab({
  ...
  async function resendInvitation(invitationId: number, e: React.MouseEvent) {
    ...
    const data = await res.json();
    if (data.success) { toast.success(t("invitationResent")); onRefresh(); }
    else toast.error(data.error?.message || t("errorResending"));
  }

  async function revokeInvitation(invitationId: number, e: React.MouseEvent) {
    e.stopPropagation();
    if (!await appConfirm({ title: t("revokeInvitation"), variant: "destructive", confirmLabel: t("revoke") })) return;
    const res = await fetch(`/api/invitations?id=${invitationId}`, { method: "DELETE" });
    const data = await res.json();
    if (data.success) { toast.success(t("invitationRevoked")); onRefresh(); }
    else toast.error(data.error?.message || t("errorRevoking"));
  }
  ...
  async function sendInvitation(participantId: number, e: React.MouseEvent) {
    ...
    const data = await res.json();
    if (data.success) { toast.success(t("invitationSent")); onRefresh(); }
    else toast.error(data.error?.message || t("errorInviting"));
  }

  async function removeCollaborator(participantId: number, e: React.MouseEvent) {
    e.stopPropagation();
    if (!await appConfirm({ title: t("revokeAccess"), description: t("revokeAccessDesc"), variant: "destructive", confirmLabel: t("revoke") })) return;
    const res = await fetch(`/api/events/${eventId}/collaborators/${participantId}`, { method: "DELETE" });
    const data = await res.json();
    if (data.success) { toast.success(t("accessRevoked")); onRefresh(); }
    else toast.error(data.error?.message || t("errorRevoking"));
  }
    ...
    <>
      <div style={{ padding: 16, borderRadius: "var(--r-md)", border: "1px solid var(--line-1)", background: "var(--bg-panel)" }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 14 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 600 }}>{t("eventCollaborators")}</div>
            <div style={{ fontSize: 12, color: "var(--ink-3)", marginTop: 2 }}>
              {t("eventCollaboratorsDesc")}
            </div>
          </div>
          <button
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                      ...
                      {/* Status chips */}
                      {c.invitationStatus === "active" && <Chip color="green">{t("statusActive")}</Chip>}
                      {c.invitationStatus === "collab_pending" && <Chip color="amber">{t("statusPending")}</Chip>}
                      {c.invitationStatus === "pending" && (
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          <Chip color={isExpired ? "gray" : isExpiringSoon ? "red" : "amber"}>
                            {isExpired ? t("expired") : isExpiringSoon ? t("expiringSoon") : t("statusPending")}
                          </Chip>
                          {c.invitationId && (
                            <>
                      ...
                      {c.invitationStatus === "no_email" && c.contactId && <Chip color="gray">{t("noEmail")}</Chip>}
                      {c.invitationStatus === "not_invited" && c.contactId && (
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          <Chip color="orange">{t("notInvited")}</Chip>
    ...
    <EventSectionGuard eventId={eventId} section="settings">
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "inline-flex", background: "var(--bg-subtle)", borderRadius: 999, padding: 3, gap: 2, alignSelf: "flex-start" }}>
          {([["data", t("eventData")], ["collabs", t("collaborators")]] as ["data" | "collabs", string][]).map(([k, l]) => (
            <button
              key={k}
              onClick={() => setTab(k)}
              style={{
