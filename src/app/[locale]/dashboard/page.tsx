...
// Accepts `t` so labels can be translated at call site (inside components).
function eventTypeMeta(
  ...
  switch (type) {
    case "wedding": return { label: t("eventTypes.wedding"), color: "wedding" };
    case "pre_wedding": return { label: t("eventTypes.pre_wedding"), color: "wedding" };
    case "post_wedding": return { label: t("eventTypes.post_wedding"), color: "wedding" };
    case "birthday": return { label: t("eventTypes.birthday"), color: "birthday" };
    case "corporate": return { label: t("eventTypes.corporate"), color: "corporate" };
    case "social": return { label: t("eventTypes.social"), color: "stage" };
    case "other": return { label: t("eventTypes.other"), color: "stage" };
    default: return { label: t("eventTypes.default"), color: "stage" };
  }
}
...
// LEVELS is built inside components (needs translated labels from useTranslations)
const LEVEL_DEFS = [
  { min: 0,   key: "newArrival" as const, emoji: "👋", color: "#6B7280" },
  { min: 25,  key: "starting"   as const, emoji: "🌱", color: "#3B82F6" },
  { min: 50,  key: "onTheWay"   as const, emoji: "⚡", color: "#F59E0B" },
  { min: 75,  key: "almostReady"as const, emoji: "🔥", color: "#EF4444" },
  { min: 100, key: "maxLevel"   as const, emoji: "🏆", color: "#10B981" },
];

function getLevel(pct: number, levels: { min: number; label: string; emoji: string; color: string }[]) {
  return [...levels].reverse().find((l) => pct >= l.min) ?? levels[0];
}

function ProgressRing({ pct, levels }: { pct: number; levels: { min: number; label: string; emoji: string; color: string }[] }) {
  const r = 22;
  const circ = 2 * Math.PI * r;
  const dash = (pct / 100) * circ;
  const lvl = getLevel(pct, levels);
  return (
...
function OnboardingCard({ isProvider, profileCompleteness, instagramHandle, verificationStatus, portfolioCount, invoiceCount, hasFiscalData, plannerOnboarding, router }: {
  ...
  const LEVELS = LEVEL_DEFS.map((d) => ({ ...d, label: t(`levels.${d.key}`) }));
  ...
  const providerSteps: OnboardingStep[] = [
    {
      icon: IcoUserEdit,
      label: t("onboarding.provider.step1Label"),
      sub: t("onboarding.provider.step1Sub"),
      time: t("onboarding.provider.step1Time"),
      reward: t("onboarding.provider.step1Reward"),
      done: profileCompleteness >= 60,
      href: "/dashboard/public-profile",
    },
    {
      icon: IcoImage,
      label: t("onboarding.provider.step2Label"),
      sub: t("onboarding.provider.step2Sub"),
      time: t("onboarding.provider.step2Time"),
      reward: t("onboarding.provider.step2Reward"),
      done: (portfolioCount ?? 0) > 0,
      href: "/dashboard/public-profile",
    },
    {
      icon: IcoStore,
      label: t("onboarding.provider.step3Label"),
      sub: t("onboarding.provider.step3Sub"),
      time: t("onboarding.provider.step3Time"),
      reward: t("onboarding.provider.step3Reward"),
    ...
    {
      icon: IcoInvoice,
      label: t("onboarding.provider.step4Label"),
      sub: t("onboarding.provider.step4Sub"),
      time: t("onboarding.provider.step4Time"),
      reward: t("onboarding.provider.step4Reward"),
      done: hasFiscalData ?? false,
      href: "/dashboard/settings?section=fiscal",
    },
    {
      icon: IcoInvoice,
      label: t("onboarding.provider.step5Label"),
      sub: t("onboarding.provider.step5Sub"),
      time: t("onboarding.provider.step5Time"),
      reward: t("onboarding.provider.step5Reward"),
      done: (invoiceCount ?? 0) > 0,
      href: "/dashboard/finance/invoices",
    },
    {
      icon: IcoBriefcase,
      label: t("onboarding.provider.step6Label"),
      sub: t("onboarding.provider.step6Sub"),
      time: t("onboarding.provider.step6Time"),
      reward: t("onboarding.provider.step6Reward"),
  ...
  const plannerSteps: OnboardingStep[] = [
    {
      icon: IcoEvents,
      label: t("onboarding.planner.step1Label"),
      sub: t("onboarding.planner.step1Sub"),
      time: t("onboarding.planner.step1Time"),
      reward: t("onboarding.planner.step1Reward"),
      done: (plannerOnboarding?.eventCount ?? 0) > 0,
      href: "/dashboard/events?new=true",
    },
    {
      icon: IcoUserAdd,
      label: t("onboarding.planner.step2Label"),
      sub: t("onboarding.planner.step2Sub"),
      time: t("onboarding.planner.step2Time"),
      reward: t("onboarding.planner.step2Reward"),
      done: (plannerOnboarding?.contactCount ?? 0) > 0,
      href: "/dashboard/contacts",
    },
    {
      icon: IcoInvoice,
      label: t("onboarding.planner.step3Label"),
      sub: t("onboarding.planner.step3Sub"),
      time: t("onboarding.planner.step3Time"),
      reward: t("onboarding.planner.step3Reward"),
      done: (plannerOnboarding?.quoteCount ?? 0) > 0,
      href: "/dashboard/finance/quotes",
    },
    {
      icon: IcoBriefcase,
      label: t("onboarding.planner.step4Label"),
      sub: t("onboarding.planner.step4Sub"),
      time: t("onboarding.planner.step4Time"),
      reward: t("onboarding.planner.step4Reward"),
      done: false,
  ...
  const FOOTER_MSG = [
    t("onboarding.footerMsg0"),
    t("onboarding.footerMsg1"),
    t("onboarding.footerMsg2"),
    t("onboarding.footerMsg3"),
    t("onboarding.footerMsg4"),
  ];
    ...
    return (
      ...
      >
        ...
        <div className="text-[13px] text-[var(--ink-3)] mt-1">
          <span dangerouslySetInnerHTML={{ __html: t("onboarding.celebrationSub", { xp: steps.length * XP_PER_STEP }) }} />
        </div>
      </div>
    );
  }
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          <ProgressRing pct={pct} levels={LEVELS} />
            ...
            <>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  {step.done ? (
                    <span className="text-[10.5px] font-semibold" style={{ color: "#10B981" }}>{t("onboarding.xpEarned", { xp: XP_PER_STEP })}</span>
                  ) : isNext && !showInline ? (
                    <>
                    ...
                    <div className="flex gap-1.5">
                      <div className="flex-1 flex items-center gap-1 rounded-[7px] px-2 py-1.5 text-[11.5px]" style={{ background: "var(--bg-subtle)", border: "1px solid var(--line-1)" }}>
                        ...
                        <input
                          ...
                          placeholder={t("onboarding.igPlaceholder")}
                      ...
                      >
                        {igSaved ? t("onboarding.igSaved") : igSaving ? t("onboarding.igSaving") : t("onboarding.igSave")}
                      </button>
                    </div>
                    <button
                ...
                {showInline && step.inlineAction === "request-verification" && (
                  <div className="mt-2.5 space-y-1.5" onClick={(e) => e.stopPropagation()}>
                    ...
                    >
                      {verifDone ? t("onboarding.verifDone") : verifSaving ? t("onboarding.verifSending") : t("onboarding.verifRequest")}
                    </button>
  ...
  return (
    <div className="px-4 py-3.5 border-t border-[var(--line-2)] first:border-t-0">
      ...
      <div
        ...
        style={{ letterSpacing: "0.06em" }}
      >
        {dueDate ? format(new Date(dueDate), "d MMM yyyy", { locale: es }).toUpperCase() : t("tasks.noDate")}
        {stage && ` · ${stage}`}
      </div>
      <div className="flex items-center mt-2.5">
        <span
          ...
          style={{ background: pill.bg, color: pill.ink }}
        >
          {done ? t("tasks.statusDone") : t("tasks.statusPending")}
        </span>
  ...
  if (!data) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3">
        ...
        <p className="text-[var(--ink-2)] text-sm">{t("client.noEvent")}</p>
      </div>
    );
  }

  const { event, daysLeft, progress, budget, rsvp, checklist, meetings, documents } = data;
  const meta = eventTypeMeta(event.type, t);
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: heroGradient }}>
        ...
        <div className="px-6 py-3 border-t border-black/5 flex items-center gap-6 flex-wrap">
          {daysLeft !== null && (
            <div>
              <div className="text-[10.5px] text-[var(--ink-3)] uppercase tracking-wider">{t("client.daysLeft")}</div>
              <div className="text-xl font-semibold text-[var(--ink-1)]">{daysLeft} {t("client.days")}</div>
            </div>
          )}
          {daysLeft !== null && <div className="w-px self-stretch bg-black/10" />}
          <div className="flex-1 min-w-[140px]">
            <div className="text-[10.5px] text-[var(--ink-3)] uppercase tracking-wider mb-1.5">{t("client.overallProgress")}</div>
            <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-xl bg-[var(--bg-card)] border border-[var(--line-1)] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-[var(--ink-2)]">{t("client.budgetTitle")}</span>
          </div>
          ...
          <div className="mt-2 flex justify-between text-[11.5px] text-[var(--ink-3)]">
            <span>{t("client.budgetPaid")} {fmtEur(budget.spent)}</span>
            {budget.total !== null && <span>{t("client.budgetRemaining")} {fmtEur(Math.max(0, budget.total - budget.spent))}</span>}
          </div>
        </div>
        ...
        <div className="rounded-xl bg-[var(--bg-card)] border border-[var(--line-1)] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-[var(--ink-2)]">{t("client.rsvpTitle")}</span>
          </div>
          <div className="flex gap-5">
            <div>
              <div className="text-[22px] font-semibold text-[var(--ink-1)]">{rsvp.confirmed}</div>
              <div className="text-[11.5px] text-[var(--ink-3)]">{t("client.rsvpConfirmed")}</div>
            </div>
            <div>
              <div className="text-[22px] font-semibold text-[var(--ink-1)]">{rsvp.pending}</div>
              <div className="text-[11.5px] text-[var(--ink-3)]">{t("client.rsvpPending")}</div>
            </div>
            <div>
              <div className="text-[22px] font-semibold text-[var(--ink-1)]">{rsvp.declined}</div>
              <div className="text-[11.5px] text-[var(--ink-3)]">{t("client.rsvpDeclined")}</div>
            </div>
          </div>
          {rsvp.total > 0 && (
            <div className="mt-3 h-1.5 bg-[var(--bg-subtle)] rounded-full overflow-hidden">
        ...
        <div className="rounded-xl bg-[var(--bg-card)] border border-[var(--line-1)] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-[var(--ink-2)]">{t("client.checklistTitle")}</span>
          </div>
          {checklist.length === 0 ? (
            <p className="text-[13px] text-[var(--ink-3)]">{t("client.checklistEmpty")}</p>
          ) : (
            <div className="flex flex-col gap-2">
              ...
              {checklist.length > 3 && (
                <p className="text-[11.5px] text-[var(--ink-3)] mt-1">{t("client.checklistMore", { n: checklist.length - 3 })}</p>
              )}
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl bg-[var(--bg-card)] border border-[var(--line-1)] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-[var(--ink-2)]">{t("client.meetingsTitle")}</span>
          </div>
          {meetings.length === 0 ? (
            <p className="text-[13px] text-[var(--ink-3)]">{t("client.meetingsEmpty")}</p>
          ) : (
        ...
        <div className="rounded-xl bg-[var(--bg-card)] border border-[var(--line-1)] p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-[var(--ink-2)]">{t("client.documentsTitle")}</span>
          </div>
          {documents.length === 0 ? (
            <p className="text-[13px] text-[var(--ink-3)]">{t("client.documentsEmpty")}</p>
          ) : (
            <div className="flex flex-col divide-y divide-[var(--line-2)]">
              {documents.map((doc) => {
...
export default function DashboardPage() {
  ...
  const [loading, setLoading] = useState(true);
  const [tab, setTab] = useState<"events" | "payments" | "tasks">("events");
    ...
    <div className="flex flex-col gap-[18px]">
      ...
      {showClaimedBanner && (
        ...
        >
          ...
          <button
            ...
            style={{ color: "var(--success-ink)" }}
            aria-label={t("claimedBanner.close")}
          >
            <IcoClose size={16} />
          </button>
        </div>
      ...
      >
        <Kpi
          ...
          sub={isProvider ? t("kpi.activeEventsSubProvider") : t("kpi.activeEventsSubPlanner")}
        />
        <Kpi
          label={t("kpi.pendingTasks")}
          value={kpiTasks}
          animateValue={loading ? undefined : kpiTasks}
          delta={kpiTasks > 0 ? `${kpiTasks} ${kpiTasks > 1 ? t("kpi.pendingTasksSub") : t("kpi.deltaUpToDate")}` : t("kpi.deltaUpToDate")}
          deltaTone={kpiTasks > 0 ? "neg" : "zero"}
          sub={t("kpi.pendingTasksSub")}
        />
        <Kpi
          label={isProvider ? t("kpi.pendingInvoices") : t("kpi.pendingPayments")}
          value={isProvider ? kpiPaymentsAmt : fmtEur(kpiPaymentsAmt, cur)}
          delta={kpiPaymentsAmt > 0 ? t("kpi.deltaCollect") : t("kpi.deltaUpToDate")}
          deltaTone={kpiPaymentsAmt > 0 ? "neg" : "zero"}
          sub={t("kpi.pendingPaymentsSub")}
        />
        <Kpi
          label={isProvider ? t("kpi.totalRevenue") : t("kpi.activeLeads")}
          value={isProvider ? fmtEur(kpiLeadsOrRev, cur) : kpiLeadsOrRev}
          animateValue={!isProvider && !loading ? kpiLeadsOrRev : undefined}
          delta={kpiLeadsOrRev > 0 ? t("kpi.deltaPipeline") : t("kpi.deltaNoMovement")}
          deltaTone={kpiLeadsOrRev > 0 ? "pos" : "zero"}
          sub={isProvider ? t("kpi.revenueSubProvider") : t("kpi.leadsSubPlanner")}
        />
      </div>
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-3.5">
          <DashCard>
            <div className="text-[13px] text-[var(--ink-3)] font-medium mb-1">
              {isProvider ? t("balance.collectedThisYear") : t("balance.totalBalance")}
            </div>
            ...
            {sparkSeries.length > 0 && sparkSeries.some((s) => s.total > 0) && (
              <div className="flex justify-between mt-1.5 text-[11px] text-[var(--ink-4)]">
                {(() => {
                  const monthsAbbr = t.raw("balance.monthsAbbr") as string[];
                  const months = sparkSeries.map((s) => {
                    const [, m] = s.month.split("-");
                    return monthsAbbr[Number(m)] || "";
                  });
          ...
          {isProvider && prov && (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-[13px] font-semibold text-[var(--ink-1)]">{t("publicProfile.title")}</span>
              ...
              <div className="mb-1 flex items-center justify-between">
                <span className="text-[11.5px] text-[var(--ink-3)]">{t("publicProfile.completeness")}</span>
              ...
              <div className="flex gap-4">
                <div>
                  ...
                  <div className="text-[11px] text-[var(--ink-3)]">{t("publicProfile.reviews")}</div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-[11px] text-[var(--ink-3)]">{t("publicProfile.events")}</div>
                </div>
                <div className="w-px self-stretch bg-[var(--line-1)]" />
                <div>
                  <div className="text-[18px] font-semibold text-[var(--ink-1)]">{fmtEur(prov.stats.totalRevenue, prov.stats.currency)}</div>
                  <div className="text-[11px] text-[var(--ink-3)]">{t("publicProfile.collected")}</div>
                </div>
              </div>
            </div>
          )}
          ...
          {isProvider ? (
            <>
              <ActionRow icon={IcoTasks} title={t("actions.providerTasks")}
                sub={`${kpiTasks} ${kpiTasks !== 1 ? t("actions.providerTasksPendingPlural") : t("actions.providerTasksPending")}`}
                onClick={() => router.push("/dashboard/tasks")} />
              <ActionRow icon={IcoWallet} title={t("actions.providerInvoices")}
                sub={`${kpiPaymentsAmt} ${kpiPaymentsAmt !== 1 ? t("actions.providerInvoicesOpenPlural") : t("actions.providerInvoicesOpen")}`}
                onClick={() => router.push("/dashboard/finance/invoices")} />
              <ActionRow icon={IcoEvents} title={t("actions.providerEvents")}
                sub={`${kpiEvents} ${t("actions.providerEventsParticipation")}`}
                onClick={() => router.push("/dashboard/events")} />
            </>
          ) : (
            <>
              <ActionRow icon={IcoEvents} title={t("actions.plannerNewEvent")} sub={t("actions.plannerNewEventSub")}
                onClick={() => router.push("/dashboard/events?new=true")} />
              <ActionRow icon={IcoUser} title={t("actions.plannerAddLead")} sub={t("actions.plannerAddLeadSub")}
                onClick={() => router.push("/dashboard/crm?new=true")} />
              <ActionRow icon={IcoTasks} title={t("actions.plannerNewTask")} sub={t("actions.plannerNewTaskSub")}
                onClick={() => router.push("/dashboard/tasks?new=true")} />
              <ActionRow icon={IcoMoney} title={t("actions.plannerRegisterPayment")} sub={t("actions.plannerRegisterPaymentSub")}
        ...
        <div className="flex flex-col gap-3.5">
          <DashCard
            title={t("nextEvent.title")}
            icon={IcoEvents}
            action={t("nextEvent.viewEvent")}
            onAction={() => {
          ...
          >
            {loading ? (
              <div className="text-[13px] text-[var(--ink-3)] py-4">{t("nextEvent.loading")}</div>
            ) : nextEvent ? (
              <>
                <div className="text-[12.5px] text-[var(--ink-3)] mb-1 mt-1">{t("nextEvent.eventNameLabel")}</div>
                <div className="flex items-center justify-between mb-3">
                  <span
                  ...
                  {(() => {
                    const meta = eventTypeMeta(nextEvent.type, t);
                ...
                <div className="mb-3.5">
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="text-[11.5px] text-[var(--ink-3)]">{t("nextEvent.taskProgress")}</div>
                ...
                <div className="grid grid-cols-2 gap-x-5 gap-y-3.5 text-[13px]">
                  <div>
                    <div className="text-[11.5px] text-[var(--ink-3)] mb-1.5">{t("nextEvent.createdBy")}</div>
                    <div className="flex items-center gap-1.5">
                  ...
                  <div>
                    <div className="text-[11.5px] text-[var(--ink-3)] mb-1.5">{t("nextEvent.client")}</div>
                    <div className="flex items-center gap-1.5">
                  ...
                  <div>
                    <div className="text-[11.5px] text-[var(--ink-3)] mb-1.5">{t("nextEvent.eventDate")}</div>
                    <div className="text-[var(--ink-1)]">
                      {nextEvent.date ? format(new Date(nextEvent.date), "d 'de' MMMM, yyyy", { locale: es }) : "—"}
                    </div>
                  </div>
                  <div>
                    <div className="text-[11.5px] text-[var(--ink-3)] mb-1.5">{t("nextEvent.guests")}</div>
              ...
              <div className="py-9 text-center">
                ...
                <div className="text-[14px] font-semibold text-[var(--ink-2)] mt-2.5">{t("nextEvent.noEvents")}</div>
                <button
                  onClick={() => router.push("/dashboard/events?new=true")}
                  className="text-[13px] text-[var(--color-primary)] hover:underline mt-1.5 cursor-pointer bg-transparent border-none"
                >
                  {t("nextEvent.createFirst")}
                </button>
              </div>
            )}
          </DashCard>
          ...
          <DashCard
            ...
            action={
              <span className="inline-flex items-center gap-1">
                <IcoPlus className="h-3 w-3" /> {t("upcomingTasks.add")}
              </span>
            }
            onAction={() => router.push("/dashboard/tasks?new=true")}
          >
            {loading ? (
              <div className="text-[13px] text-[var(--ink-3)] py-2">{t("upcomingTasks.loading")}</div>
            ) : unifiedPendingTasks && unifiedPendingTasks.length > 0 ? (
              <div className="flex flex-col">
                {unifiedPendingTasks.slice(0, 3).map((task) => (
                  <div key={task.id} className="flex items-start gap-2.5 py-2.5 border-t border-[var(--line-2)] first:border-t-0 first:pt-0">
                    <button
                      ...
                      aria-label={t("upcomingTasks.markDone")}
                    />
                    <div className="flex-1 min-w-0">
                      <div className="text-[13.5px] font-medium text-[var(--ink-1)]">{task.title}</div>
              ...
              <div className="text-[13px] text-[var(--ink-3)] py-3">{t("upcomingTasks.noTasks")}</div>
            )}
          </DashCard>
        </div>
        ...
        >
          ...
          >
            {(["events", "payments", "tasks"] as const).map((tabKey) => (
              <button
                key={tabKey}
                ...
                style={{
                  ...
                  color: tab === tabKey ? "var(--ink-1)" : "var(--ink-3)",
                  fontWeight: tab === tabKey ? 600 : 500,
                  boxShadow: tab === tabKey ? "0 1px 2px rgba(0,0,0,0.05)" : "none",
                  padding: "7px 12px",
                }}
              >
                {tabKey === "events" && <IcoEvents className="h-[13px] w-[13px]" />}
                {tabKey === "payments" && <IcoMoney className="h-[13px] w-[13px]" />}
                {tabKey === "tasks" && <IcoTasks className="h-[13px] w-[13px]" />}
                {tabKey === "events" && t("schedule.tabEvents")}
                {tabKey === "payments" && t("schedule.tabPayments")}
                {tabKey === "tasks" && t("schedule.tabTasks")}
              </button>
            ))}
          </div>
          ...
          {(() => {
            ...
            if (tab === "events") {
              ...
              emptyMsg = t("schedule.noEvents");
            } else if (tab === "tasks") {
              all = (unifiedPendingTasks || [])
                .filter((task) => !search || task.title.toLowerCase().includes(search.toLowerCase()))
                .map((task) => ({ kind: "task" as const, item: task }));
              emptyMsg = t("schedule.noTasks");
            } else {
              all = (unifiedInvoices || [])
                .filter(
                  (p) =>
              ...
              emptyMsg = t("schedule.noPayments");
            }
            ...
            const renderItem = (entry: AnyItem) => {
              if (entry.kind === "event") {
                const e = entry.item;
                const meta = eventTypeMeta(e.type, t);
                return (
                  <EventMini
                    key={`ev-${e.id}`}
                    name={e.name}
            ...
            return (
              <div className="rounded-[8px]" style={{ border: "1px solid var(--line-1)" }}>
                ...
                {all.length > 3 && (
                  ...
                  >
                    {expanded ? t("schedule.showLess") : t("schedule.showMore", { n: hiddenCount })}
                    <span
