  ...
  const handleRequestVerification = async () => {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        toast.success(t("toast.verifSent"));
      } else {
        toast.error(json.error?.message || t("toast.verifError"));
      }
    } catch {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        const msg = json.error?.details
          ...
          : json.error?.message || t("toast.errorSave");
        setSaveError(msg);
        toast.error(msg);
      }
    } catch {
      setSaveError(t("toast.connectionError"));
      toast.error(t("toast.connectionError"));
    } finally {
      setSaving(false);
    }
      ...
      <div style={{ maxWidth: 860, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexShrink: 0, flexWrap: "wrap" }}>
            ...
            {!isVerified && (
              isPending ? (
                ...
                >
                  <HugeiconsIcon icon={Shield01Icon} size={13} strokeWidth={1.5} />
                  {requestingVerif ? t("verifSending") : t("requestVerif")}
                </button>
              )
            )}
            <a
              ...
              title={isVerified ? undefined : t("previewTooltip")}
            >
              <HugeiconsIcon icon={ArrowUpRight01Icon} size={13} strokeWidth={1.5} />
              {isVerified ? t("viewPublicProfile") : t("preview")}
            </a>
            <button
              onClick={handleSave}
        ...
        {hasChanges && (
          <div style={{
            ...
            <span style={{ flex: 1 }}>{t("unsavedChanges")}</span>
            <button
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <span style={{ fontSize: 14, fontWeight: 600, color: "var(--ink-1)" }}>{t("completeness")}</span>
              <span style={{
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              <span style={{
                ...
                {isVerified ? t("verified") : isPending ? t("inReview") : t("unverified")}
              </span>
              <span style={{
                display: "inline-flex", alignItems: "center", gap: 5,
                fontSize: 11, fontWeight: 500, padding: "3px 10px", borderRadius: 999,
        ...
        {/* Información básica ──────────────────────────────────────────────────── */}
        <SectionCard icon={Store02Icon} title={t("sections.basicInfo")} subtitle={t("sections.basicInfoDescription")}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <Field label={t("fields.tagline")} hint={t("fields.taglineHint")}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "minmax(0,260px) 1fr", gap: 14, alignItems: "start" }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, minHeight: 160 }}>
                  ...
                  <div style={{ fontSize: 10.5, color: "var(--ink-3)" }}>{t("fields.coverHint")}</div>
                </div>
              </div>
            </div>
            ...
            <div style={{ borderTop: "1px solid var(--line-1)", paddingTop: 14, marginTop: 2 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 0, border: "1px solid var(--line-strong)", borderRadius: "var(--r-sm)", overflow: "hidden", background: "white" }}>
                ...
                {slugStatus === "available" && (
                  <span style={{ padding: "0 12px", fontSize: 11, color: "#3F6B4D", fontWeight: 500 }}>{t("slugAvailable")}</span>
                )}
                {slugStatus === "taken" && (
                  <span style={{ padding: "0 12px", fontSize: 11, color: "#B54632", fontWeight: 500 }}>{t("slugTakenStatus")}</span>
                )}
              </div>
        ...
        {/* Contacto ────────────────────────────────────────────────────────────── */}
        <SectionCard icon={PhoneCheckIcon} title={t("sections.contact")} subtitle={t("sections.contactSubtitle")}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Field label={t("fields.phone")}>
              <input value={getValue("phone")} onChange={(e) => updateField("phone", e.target.value)} placeholder={t("fields.phonePlaceholder")} style={inputStyle} />
            </Field>
            <Field label={t("fields.publicEmail")}>
              <input type="email" value={getValue("publicEmail")} onChange={(e) => updateField("publicEmail", e.target.value)} placeholder={t("fields.publicEmailPlaceholder")} style={inputStyle} />
            </Field>
            ...
            <Field label={t("fields.instagram")} hint={t("fields.instagramHint")}>
              <div style={{ position: "relative" }}>
                <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", fontSize: 14, fontWeight: 500, color: "#E1306C" }}>@</span>
                <input value={getValue("instagramHandle")} onChange={(e) => updateField("instagramHandle", e.target.value)} placeholder={t("fields.instagramPlaceholder")} style={{ ...inputStyle, paddingLeft: 28 }} />
              </div>
            </Field>
          </div>
        </SectionCard>
        ...
        {/* Ubicación ───────────────────────────────────────────────────────────── */}
        <SectionCard icon={Location01Icon} title={t("sections.location")} subtitle={t("sections.locationSubtitle")}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            <Field label={t("fields.city")}>
              <input value={getValue("city")} onChange={(e) => updateField("city", e.target.value)} placeholder={t("fields.cityPlaceholder")} style={inputStyle} />
            </Field>
            <Field label={t("fields.region")}>
              <input value={getValue("region")} onChange={(e) => updateField("region", e.target.value)} placeholder={t("fields.regionPlaceholder")} style={inputStyle} />
            </Field>
            <Field label={t("fields.country")}>
              <input value={getValue("country")} onChange={(e) => updateField("country", e.target.value)} placeholder={t("fields.countryPlaceholder")} style={inputStyle} />
            </Field>
          </div>
        </SectionCard>
            ...
            {getInstagramPosts().map((url, idx) => (
              <div key={idx} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                {url && !isInstagramPostUrl(url) && (
                  <span style={{ fontSize: 11, color: "#B54632", flexShrink: 0 }}>{t("invalidUrl")}</span>
                )}
                <button
                  onClick={() => {
        ...
        <SectionCard
          ...
          headerRight={
            <span style={{ fontSize: 11, color: "var(--ink-3)", background: "var(--bg-subtle)", padding: "3px 9px", borderRadius: 999, fontWeight: 500 }}>
              {t("portfolio.photosCount", { count: portfolioItems.length })}
            </span>
          }
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {portfolioLoading ? (
              <div style={{ padding: 20, textAlign: "center", fontSize: 12, color: "var(--ink-3)" }}>{t("loading")}</div>
            ) : (
              <>
                ...
                {portfolioItems.length === 0 && !addingPortfolio && (
                  <div style={{ padding: 24, textAlign: "center", fontSize: 12, color: "var(--ink-3)", border: "1px dashed var(--line-1)", borderRadius: 8 }}>
                    {t("portfolio.empty")}<br />
                    <span style={{ fontSize: 11 }}>{t("portfolio.emptyHint")}</span>
                  </div>
                )}
                ...
                {addingPortfolio ? (
                  <div style={{ border: "1px solid var(--line-1)", borderRadius: 8, padding: 14, background: "var(--bg-subtle)", display: "flex", flexDirection: "column", gap: 10 }}>
                    <div style={{ fontSize: 12.5, fontWeight: 600, color: "var(--ink-1)" }}>{t("portfolio.newPhoto")}</div>
                    <Field label={t("portfolio.imageUrl")}>
                      <input
                        value={newItemUrl}
                    ...
                    <Field label={t("portfolio.titleOptional")}>
                      <input
                        value={newItemTitle}
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (res.ok) {
                              ...
                              toast.error(t("toast.errorAddPhoto"));
                            }
                          } catch {
                            toast.error(t("toast.connectionError"));
                          } finally {
                      ...
                      >
                        {newItemSubmitting ? t("saving") : t("portfolio.addPhoto")}
                      </button>
                      <button
        ...
        {/* Brochure ────────────────────────────────────────────────────────────── */}
        <SectionCard icon={File01Icon} title={t("sections.brochure")} subtitle={t("sections.brochureDescription")}>
          {getBrochureUrl() ? (
            <div style={{ display: "flex", gap: 12, alignItems: "center", padding: 14, background: "var(--bg-subtle)", border: "1px solid var(--line-1)", borderRadius: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "var(--ink-1)" }}>{t("brochureUploaded")}</div>
        ...
        {(profile.totalReviews ?? 0) > 0 && (
          <SectionCard icon={StarIcon} title={t("sections.reviews")} subtitle={t("sections.reviewsSubtitle")}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <span style={{ fontSize: 12, color: "var(--ink-3)" }}>
                {profile.totalReviews} {(profile.totalReviews ?? 0) > 1 ? t("reviewsPlural") : t("reviews")}
              </span>
            </div>
          </SectionCard>
        )}
