...
function PartnersContent() {
  ...
  const handleInviteToEvent = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(`${inviteTarget.name} ${t("invitedToEvent")}`);
      } else if (data.error?.code === "DUPLICATE") {
        toast.info(t("alreadyInvited"));
      } else {
        toast.error(data.error?.message || t("inviteError"));
      }
    } catch {
  ...
  const handleResendClaim = async (claim: PendingClaim) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(`${t("resentTo")} ${claim.email}`);
        fetchPendingClaims();
      } else {
        toast.error(data.error?.message || t("resendError"));
      }
    } catch {
      toast.error(t("connectionError"));
    } finally {
  ...
  const toggleFavorite = async (provider: PartnersListing) => {
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(data?.error?.message || t("favoriteError"));
        return;
      }
      setProviders((prev) =>
        prev.map((p) => (p.id === provider.id ? { ...p, isFavorite: !p.isFavorite } : p)),
      );
      if (!wasFav) toast.success(`${provider.name} ${t("addedToContacts")}`);
      else toast.success(`${provider.name} ${t("removedFromContacts")}`);
    } catch {
  ...
  return (
    <div className="flex flex-col gap-3.5">
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 12.5, fontWeight: 600, color: "#92400e" }}>
              {t("pendingClaims", { count: pendingClaims.length })}
            </span>
          </div>
              ...
              return (
                <div key={claim.id} style={{
                  ...
                  >
                    ...
                    {resendingId === claim.id ? t("sending") : t("resend")}
                  </button>
                </div>
              );
            })}
      ...
      {total > 50 && (
        <div className="flex items-center justify-between mt-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[12px] text-[var(--ink-3)]">
              {t("page")} {page} {t("of")} {Math.ceil(total / 50)}
            </span>
            <button
      ...
      >
        ...
        >
          <SheetTitle className="sr-only">Invitar colaborador</SheetTitle>
          <div className="flex items-start gap-3 px-6 pt-5 pb-3">
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[12.5px] text-[var(--ink-3)] mt-0.5">
                {t("inviteSubtitle")} <strong className="text-[var(--ink-1)]">{inviteTarget?.name}</strong> {t("inviteSubtitleEnd")}
              </div>
            </div>
            <button
          ...
          <div className="px-6 py-4 flex flex-col gap-3">
            <div className="drawer-form-field flex flex-col gap-1.5">
              <label className="text-[12px] font-medium text-[var(--ink-2)]">{t("selectEvent")}</label>
              <select
              ...
              {events.length === 0 && (
                <p className="text-[11px] text-[var(--ink-3)]">{t("noEvents")}</p>
              )}
            </div>
          </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        {reviews > 0 && rating > 0 ? (
          <>
            ...
            <span style={{ fontSize: 11.5, color: "var(--ink-3)" }}>· {reviews} {t("reviews")}</span>
          </>
        ) : (
          <span style={{ fontSize: 11, color: "var(--ink-4)" }}>{t("noReviews")}</span>
        )}
      </div>
...
function CreateProviderDrawer({
  ...
  const handleSubmit = async () => {
    ...
    if (!hasEmail && (!hasInstagram || !hasPhone)) {
      setFormError(t("formErrorNoEmail"));
      return;
    }
    if (isGeneric && !hasInstagram && !hasPhone) {
      setFormError(t("formErrorGenericEmail"));
      return;
    }
    setFormError("");
    setLoading(true);
    try {
      ...
      if (res.status === 409 && data.error?.code === "DUPLICATE_PROVIDER") {
        ...
        toast.error(
          ep?.name
            ? `${t("duplicateProviderName")} ${ep.name}${ep.city ? ` (${ep.city})` : ""}`
            : t("duplicateProvider"),
          { duration: 5000 }
        );
        return;
      ...
      if (!res.ok) {
        const msg = data.error?.message || t("createError");
        setFormError(msg);
        return;
      }
      toast.success(
        data.data?.invitationSent
          ? `${form.name} ${t("addedNotified")}`
          : `${form.name} ${t("addedToContacts")}`,
      );
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        <SheetTitle className="sr-only">Nuevo proveedor</SheetTitle>
        <div className="flex items-start gap-3 px-6 pt-5 pb-3">
          <div className="flex-1 min-w-0">
            <div
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-4 flex flex-col gap-3">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
            ...
            <div>
              <div style={{ fontSize: 12.5, fontWeight: 500, color: "var(--ink-1)" }}>{t("photoOrLogo")}</div>
          ...
          <div className="drawer-form-field flex flex-col gap-1.5">
            <label className="text-[12px] font-medium text-[var(--ink-2)]">{t("providerNameLabel")}</label>
            <input
              value={form.name}
          ...
          <div className="drawer-form-field flex flex-col gap-1.5">
            <label className="text-[12px] font-medium text-[var(--ink-2)]">{t("categoryLabel")}</label>
          ...
          <div className="drawer-form-field flex flex-col gap-1.5">
            <label className="text-[12px] font-medium text-[var(--ink-2)]">{t("shortDescription")}</label>
            <textarea
              rows={2}
          ...
          <div className="grid grid-cols-2 gap-2.5">
            ...
            <div className="drawer-form-field flex flex-col gap-1.5">
              <label className="text-[12px] font-medium text-[var(--ink-2)]">{t("professionalEmail")}</label>
              <input
                type="email"
                value={form.email}
          ...
          {isGeneric && (
            <div style={{
              ...
              <span>
                <strong>{t("genericEmailWarningTitle")}</strong> {t("genericEmailWarningBody")}
              </span>
            </div>
          )}
          ...
          {!hasEmail && form.email.length === 0 && (
            <div style={{
              ...
              <span>{t("noEmailWarning")}</span>
            </div>
          )}

          <div className="grid grid-cols-2 gap-2.5">
            <div className="drawer-form-field flex flex-col gap-1.5">
              <label className="text-[12px] font-medium text-[var(--ink-2)]">{t("cityLabel")}</label>
              <input
                value={form.city}
        ...
        >
          ...
          >
            {loading ? t("creating") : t("createProviderBtn")}
          </button>
        </div>
      </SheetContent>
    </Sheet>
