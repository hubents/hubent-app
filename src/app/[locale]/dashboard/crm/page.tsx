...
function SortDropdown({ value, onChange }: { value: SortKey; onChange: (v: SortKey) => void }) {
  ...
  const SORT_LABELS: Record<SortKey, string> = {
    ...
    "name-az": t("sortNameAz"),
    "name-za": t("sortNameZa"),
    "value-desc": t("sortValueDesc"),
    "value-asc": t("sortValueAsc"),
    "date-asc": t("sortDateAsc"),
    "date-desc": t("sortDateDesc"),
  };

  const groups: { group: string; items: { v: SortKey; l: string }[] }[] = [
    { group: t("sortGroupName"), items: [{ v: "name-az", l: t("sortNameAz") }, { v: "name-za", l: t("sortNameZa") }] },
    { group: t("sortGroupBudget"), items: [{ v: "value-desc", l: t("sortMayorAMenor") }, { v: "value-asc", l: t("sortMenorAMayor") }] },
    { group: t("sortGroupDate"), items: [{ v: "date-asc", l: t("sortDateAsc") }, { v: "date-desc", l: t("sortDateDesc") }] },
  ];
...
function CRMContent() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        <Kpi
          label={t("kpiPipelineTotal")}
          value={loading ? "—" : fmtMoneyShort(pipelineTotal)}
          delta={`${allLeads.length} ${t("kpiOpportunities")}`}
          deltaTone={pipelineTotal > 0 ? "pos" : "zero"}
          sub={t("kpiPipelineSub")}
        />
        <Kpi
          label={t("kpiActiveLeads")}
          value={loading ? "—" : activeLeads.length}
          delta={activeLeads.length > 0 ? t("kpiInPipeline") : t("kpiNoLeads")}
          deltaTone={activeLeads.length > 0 ? "pos" : "zero"}
          sub={t("kpiActiveLeadsSub")}
        />
        <Kpi
          label={t("kpiWonValue")}
          value={loading ? "—" : fmtMoneyShort(wonValue)}
          delta={`${wonLeads.length} ${wonLeads.length !== 1 ? t("kpiWonClosedPlural") : t("kpiWonClosed")}`}
        ...
        <Kpi
          ...
          delta={closed > 0 ? `${wonLeads.length}/${closed} ${t("kpiConvClosed")}` : t("kpiNoData")}
      ...
      >
        ...
        {loading ? (
          <div className="text-[13px] text-[var(--ink-3)] py-12 text-center">{t("loading")}</div>
        ) : filteredStages.length === 0 ? (
          <div className="text-[13px] text-[var(--ink-3)] py-12 text-center">
            {t("configurePipelineEmpty")}
          </div>
        ) : (
                ...
                >
                  <div className="funnel__arrow">
                    ...
                    <div className="funnel__meta">
                      ...
                      {stageLeads.length} {t("opportunitiesOf")} {totalAll} {t("kpiOpportunities")}
                    </div>
                  </div>
                  <div className="funnel__body">
                            ...
                            {(() => {
                              ...
                              return (
                                <span
                                  ...
                                  title={`${d.days} ${d.days !== 1 ? t("daysInStagePlural") : t("daysInStage")}`}
                                >
                    ...
                    {stageLeads.length === 0 && (
                      <div className="funnel-card__empty">{t("dragHere")}</div>
                    )}
                  </div>
                </div>
              );
      ...
      <CreateEventDrawer
        ...
        onEventCreated={(created) => {
          ...
          if (leadTitle) toast.success(t("eventCreated", { title: leadTitle }));
          if (created?.id) {
