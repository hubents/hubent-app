...
export function CalendarPageContent({
  ...
  const rangeLabel =
    rangeMode === "next7" ? t("range.next7") :
    rangeMode === "next30" ? t("range.next30") :
    t("range.week");
  ...
  const tabs: Array<{ id: "todos" | FilterKey; label: string; Icon: typeof RiCalendarLine; count: number }> = [
    { id: "todos",     label: t("filters.allScheduled"), Icon: RiCalendarLine,          count: counts.todos },
    { id: "reuniones", label: t("filters.meetings"),     Icon: RiPhoneLine,             count: counts.reuniones },
    { id: "eventos",   label: t("filters.events"),       Icon: RiCalendarEventLine,     count: counts.eventos },
    { id: "pagos",     label: t("filters.payments"),     Icon: RiMoneyDollarCircleLine, count: counts.pagos },
  ];
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center" style={{ gap: 8, flexWrap: "nowrap" }}>
        ...
        <BtnIcon
          ...
          aria-label={t("toolbar.prevMonth")}
        >
        ...
        <div ref={rangeRef} style={{ position: "relative" }}>
          ...
          {rangeOpen && (
            ...
            >
              {([
                { v: "week",   l: t("range.week") },
                { v: "next7",  l: t("range.next7") },
                { v: "next30", l: t("range.next30") },
              ] as const).map((o) => {
        ...
        {/* Actions */}
        {!eventId && (
          <button style={pillGhostStyle} title={t("toolbar.schedule")} onClick={() => setScheduleOpen(true)}>
            <RiCalendarLine className="h-3.5 w-3.5" /> {t("toolbar.schedule")}
          </button>
        )}
        {!eventId && (
          <button style={pillPrimaryStyle} onClick={() => setCreateOpen(true)} title={t("toolbar.createEvent")}>
            <RiAddLine className="h-3.5 w-3.5" /> {t("toolbar.createEvent")}
          </button>
        )}
        <BtnIcon title={t("toolbar.settings")} aria-label={t("toolbar.settings")}>
      ...
      {undatedEvents.length > 0 &&
        (activeFilters.size === 0 || activeFilters.has("eventos")) && (
          ...
          >
            ...
            >
              {t("undated.label", { count: undatedEvents.length })}
            </span>
  ...
  const badgeLabel =
    daysDiff === 0
      ...
      : daysDiff === 1
        ...
        : daysDiff > 1
          ? t("highlight.inDays", { n: daysDiff })
          : t("highlight.daysAgo", { n: Math.abs(daysDiff) });
