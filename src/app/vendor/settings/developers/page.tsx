...
export default function VendorDevelopersPage() {
  ...
  const handleCreate = async () => {
    if (!newKeyName.trim()) { toast.error("El nombre es obligatorio"); return; }
    ...
    try {
      const res = await fetch("/api/settings/api-keys", {
        ...
        body: JSON.stringify({ name: newKeyName, scopes: newKeyScopes, environment: newKeyEnvironment }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleRevoke = async (id: number) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { toast.success("API key revocada"); fetchKeys(); }
      else { toast.error(data.error?.message || "Error al revocar"); }
    } catch { toast.error("Error al revocar API key"); }
  };

  const copyToClipboard = async (text: string, keyId?: number) => {
    await navigator.clipboard.writeText(text);
    if (keyId) { setCopiedKeyId(keyId); setTimeout(() => setCopiedKeyId(null), 2000); }
  ...
  const filteredScopeGroups = SCOPE_GROUPS
    .map((g) => ({ ...g, scopes: availableScopes.length > 0 ? g.scopes.filter((s) => availableScopes.includes(s)) : g.scopes }))
    .filter((g) => g.scopes.length > 0);

  const selectAllScopes = () => { setNewKeyScopes(filteredScopeGroups.flatMap((g) => g.scopes)); };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        <Link href="/vendor/settings" className="text-muted-foreground hover:text-foreground">
          <RiArrowLeftSLine className="h-5 w-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-semibold">API Keys</h1>
          <p className="text-sm text-muted-foreground">Gestiona API keys para integrar con la API publica de HubEnts</p>
        </div>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-lg bg-red-500/10"><RiShieldLine className="h-5 w-5 text-red-500" /></div>
                <div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>Nombre</Label>
                <Input placeholder="Mi integracion..." value={newKeyName} onChange={(e) => setNewKeyName(e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>Entorno</Label>
                <div className="flex gap-2">
                  <Button size="sm" variant={newKeyEnvironment === "live" ? "default" : "outline"} onClick={() => setNewKeyEnvironment("live")}>Live</Button>
                  <Button size="sm" variant={newKeyEnvironment === "test" ? "default" : "outline"} onClick={() => setNewKeyEnvironment("test")}>Test</Button>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" variant="ghost" onClick={selectAllScopes} className="text-xs">Seleccionar todos</Button>
              </div>
            ...
            <div className="flex gap-2 pt-2">
              <Button onClick={handleCreate} disabled={creating}>{creating ? "Creando..." : "Crear API Key"}</Button>
        ...
        <CardContent>
          {keys.length === 0 ? (
            ...
            <div className="space-y-3">
              {keys.map((key) => (
                <div key={key.id} className={`border rounded-lg p-4 ${key.isActive ? "" : "opacity-60"}`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => copyToClipboard(key.keyPrefix, key.id)}>
                          {copiedKeyId === key.id ? <RiCheckLine className="h-3 w-3 text-green-500" /> : <RiFileCopyLine className="h-3 w-3" />}
                        </Button>
                      </div>
                      <div className="flex flex-wrap gap-1 mb-2">
                        ...
                        {key.scopes.length > 5 && <Badge variant="outline" className="text-[10px]">+{key.scopes.length - 5} mas</Badge>}
                      </div>
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        ...
                        {key.lastUsedAt && <span>Ultimo uso: {new Date(key.lastUsedAt).toLocaleDateString()}</span>}
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <a href="/api/v1/openapi" target="_blank" className="flex items-center gap-2 border rounded-lg p-3 hover:bg-muted/50 transition-colors">
            ...
            <a href="/developers" target="_blank" className="flex items-center gap-2 border rounded-lg p-3 hover:bg-muted/50 transition-colors">
              <RiExternalLinkLine className="h-4 w-4 text-muted-foreground" />
              <div>
