...
export function useTaskDetail(taskId: number | null) {
  ...
  const addParticipant = useCallback(async (participantData: { userId?: string; vendorId?: number; contactId?: number; providerOrgId?: number; type: string; canEdit?: boolean; canComment?: boolean }) => {
    if (!taskId) return null;
